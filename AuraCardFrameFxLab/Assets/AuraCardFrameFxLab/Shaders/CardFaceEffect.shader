Shader "SunExp/CardFaceEffect"
{
    Properties
    {
        ...
        _SunExpFrameOnlyOverlay ("Frame Only Overlay", Float) = 0
        _SunExpFoilMode ("Foil Mode", Float) = 1
        ...
        _SunExpFlowSpeed ("Flow Speed", Float) = 0.55
        _SunExpFlowScale ("Flow Scale", Float) = 1.22
        _SunExpNoiseScale ("Noise Scale", Float) = 4.0
        _SunExpDistortion ("Distortion", Float) = 0.009
        _SunExpEffectIntensity ("Effect Intensity", Range(0, 2)) = 1.04
        _SunExpQualityScale ("Quality Scale", Range(0, 1)) = 1
        _SunExpEdgeGlow ("Edge Glow", Range(0, 1)) = 0.28
        _SunExpSweepFrequency ("Sweep Frequency", Float) = 4.4
        _SunExpSweepWidth ("Sweep Width", Range(0.01, 1)) = 0.13
        _SunExpSweepIntensity ("Sweep Intensity", Range(0, 2)) = 1.12
        _SunExpPrismScale ("Prism Scale", Float) = 13.5
        _SunExpPrismStrength ("Prism Strength", Range(0, 1)) = 1
        _SunExpFoilGrain ("Foil Grain", Range(0, 1)) = 0.08
        _SunExpMirrorSweep ("Mirror Sweep", Range(0, 2)) = 0.58
        _SunExpSwirlStrength ("Swirl Strength", Range(0, 1)) = 0.06
        _SunExpFoilShardScale ("Foil Shard Scale", Float) = 18
        _SunExpFoilShardWarp ("Foil Shard Warp", Range(0, 1)) = 0.08
        _SunExpFoilGalaxyDensity ("Foil Galaxy Density", Range(0, 1)) = 0.015
        _SunExpFoilGlintSpeed ("Foil Glint Speed", Float) = 1.1
        _SunExpFoilTextureStrength ("Foil Texture Strength", Range(0, 2)) = 0.6
        _SunExpRainbowStrength ("Rainbow Strength", Range(0, 2)) = 1.25
        _SunExpRidgeStrength ("Ridge Strength", Range(0, 2)) = 0.7
        _SunExpGlareStrength ("Glare Strength", Range(0, 2)) = 0.35
        _SunExpPointerAutoSpeed ("Pointer Auto Speed", Float) = 0.78
        _SunExpFoilOverlayAlpha ("Foil Overlay Alpha", Range(0, 2)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                if (_SunExpFrameOnlyOverlay < 0.5)
                {
                    return mask;
                }
            ...
            fixed3 pokemonTextureShine(float2 uv, float time, float3 noise, float2 pointer)
            {
                ...
                textureLayer += holoRamp(uv.x * 1.1 - uv.y * 0.28 + foilGrain + time * 0.02) * foilGrain * _SunExpFoilTextureStrength * 0.62;
            ...
            fixed3 applyPokemonFoil(fixed3 baseColor, float2 uv, float mask, float time, float3 noise, float edge, float intensity)
            {
                ...
                fixed3 shineStack = colorDodgeBlend(baseColor, mainShine, 0.76 * _SunExpPrismStrength);
            ...
            fixed3 applyFoil(fixed3 baseColor, float2 uv, float mask, float time, float3 noise, float edge, float intensity)
            {
                ...
                fixed3 foilColor = lerp(baseColor, saturate(baseColor * 0.86 + holo * 1.1), _SunExpPrismStrength);
            ...
            fixed4 buildFoilOverlay(float2 uv, float mask, float time, float3 noise, float edge, float intensity)
            {
                ...
                color += _SunExpHoloColorA.rgb * (glint * 0.26 + edge * 0.2) * intensity;
                ...
                if (_SunExpFoilMode > 1.5)
                {
                    ...
                    float pokemonAlpha = saturate((pokemonSparkle * 0.98 + fromCenter * 0.12 + edge * 0.28) * intensity * _SunExpFoilOverlayAlpha);
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float overlayEdge = max(edge, innerEdge(i.uv, overlayMask) * _SunExpEdgeGlow);
                float overlayIntensity = _SunExpEffectIntensity * _SunExpQualityScale * overlayMask;
