...
- Dynamic card tags vs. card effects: adding a tag through `Vars.SpecialTag` only updates the current card instance's visible/dictionary tag. It does not make ordinary cards run keyword logic. For temporary `白曜` from `白曜圣祷`, keep the effect battle-local by marking the current `CardItem`/`DataConfig` `Vars.SunExpTempWhiteRadiance = "1"` plus `Vars.SunExpTempWhiteRadianceLockId`, trigger only from `CommonCardItem.TrueUse.after` / `AttackCardItem.TrueUse.after`, and claim `FightManager.Instance.TempVarsMap["SunExpTempWhiteRadianceResolved_" .. lockId]` before settlement. Skip cards whose base `Data/Card.Tag` already contains `白曜` to avoid native/dynamic double triggers. Do not use `CardItem.RunScript.after` or `ActionAfter` as fallback trigger routes for this path.
