...
Terrias dynamic combat cards use the native `GetCardFromDeck` queue,
initialization, draw/use/exit animations and destruction. The whole-CardItem
pool, exit layer/animator, catalog and performance toggle have been retired.
Keep resource caches and independent UI-list pooling.

The shared presentation runtime brackets native Init/DataUpdate/SetCardStyle.
Nested writes suppress intermediate reapply and commit once after the outer
refresh. Release temporary materials before native style writes.
`PrepareNativeExit` handles native and script-free replay exits: it blocks
future reapply and dispatches a `NativeExit` reset. AuraTools releases the
dynamic effect while retaining the selected static skin so native burn can
copy the displayed textures. A material refresh does not end a visual's
recording lifetime.
...
- Retained presentation-only UI reuse uses the shared lifecycle. Native combat
  cards commit through the native-write boundary; do not restore the retired
  Terrias-only materialization path.
- `AuraPresentationMaterialCoordinator` is the only temporary-material owner
  ...
  `aura-shared-runtime-dev/references/shared-mutable-runtime-ownership.md`;
  do not duplicate or weaken it in either consumer.
- The current [card-visual registry](../../../../AuraToolsExp-Dev/Features/CardVisual/AuraToolsCardVisualRegistry.cs)
  declares the protocol and native frame coverage profile. Its `frame` target
  must follow that profile:
...
- Build `auratools_visuals` with the Unity/URP versions and target declared by
  [the tracked builder](../../../../tools/Build-AuraToolsVisualBundle.ps1).
  The build project must bind a real URP Pipeline Asset before
...
- Replay card instances carry a read-only snapshot of their effective theme,
  ...
  snapshot uses the same current native-frame renderer and never rewrites player config.
...
Run the affected tool behavior and ownership checks, then build only bundles
whose tracked sources changed. Use the
[impact guide](../../aura-project-dev/references/validation.md) for publication
and the owning runtime matrix for real rendering evidence.
