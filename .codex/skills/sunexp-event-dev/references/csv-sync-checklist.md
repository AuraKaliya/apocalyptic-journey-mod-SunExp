...
- Every `SunExp_...(` call in `Data/EventList` must be defined in `Entry.lua`.
- Every such helper should be registered through `SunExp_RegisterDynamicMethods`.

## IDs

- Use full mod ids in helper arguments, for example `SunExp_sunexp_morning_shard`.
