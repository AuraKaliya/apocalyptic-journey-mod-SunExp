...
- Palette: object uses only 2-3 colors total.
  - Main light: pale warm ivory / white-hot cream.
  - Shadow/flame: saturated magenta-crimson.
  - Optional accent: small muted orange only when needed.
...
no fine details, no text, no border, no UI frame.
Visual subject: <one clear symbolic object based on the name/effect>.
Palette: near-black navy background; pale warm ivory main shape;
saturated magenta-crimson shadow/flame; optional tiny muted orange accent.
...
larger silhouette, less texture, only ivory and magenta-crimson on a dark navy background.
```
...
3. Generate preview images with `imagegen`.
4. Before replacing project assets, normalize generated images to `512x512` PNG. The image generator may ignore requested dimensions.
5. Save final card images to the exact paths referenced by `Icon` in `Data/Card/sunexp.csv`.
6. If tracked atlas/source preview images exist in `SunExp/ModResource/Images/Card/SunExp`, update them so they do not preserve stale art direction.
7. Create or update a contact sheet outside the game resource folder, for example:
...
- The set reads as one pack family on the contact sheet.
- Object colors are constrained to 2-3 colors.
