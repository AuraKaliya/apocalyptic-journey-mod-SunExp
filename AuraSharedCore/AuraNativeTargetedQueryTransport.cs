...
public static class AuraNativeTargetedQueryTransport
{
    ...
    {
        if (ReferenceEquals(manager, null) || queryId == 0 || PendingQueriesField == null)
        {
            return;
        }
        ...
        {
            AuraSharedLog.Warn("TargetedQuery", "Pending callback cleanup failed: " + ex.Message);
        }
    }

    public static bool TrySend(NetworkConnectionToClient connection, uint queryId, QueryBase response, out string rejection)
    {
        rejection = "";
        if (connection == null || queryId == 0 || response == null || RpcQueryMethod == null
            || !NetworkServer.connections.TryGetValue(connection.connectionId, out var live) || !ReferenceEquals(live, connection))
        { rejection = "targeted response connection unavailable"; return false; }
        var target = connection.identity?.GetComponent<PlayerManager>();
        if (target == null) { rejection = "targeted response player unavailable"; return false; }
        try
        {
            response.QueryId = queryId;
            RpcQueryMethod.Invoke(target, new object[] { connection, response });
            return true;
        }
        catch (Exception ex) { rejection = ex.InnerException?.Message ?? ex.Message; return false; }
    }
