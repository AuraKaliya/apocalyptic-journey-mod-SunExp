...
| `AuraCgShared` | Runtime domain arbiter | Aligned | Provides the global CG arbiter, registry-backed owner-qualified identities, priority ordering, queueing, duplicate windows, host-relayed compact playback events, fight-session isolation, and local shared-resource resolution. | Keep the compact playback protocol registry-only; resource bytes and raw presentation payloads do not cross the network. |
