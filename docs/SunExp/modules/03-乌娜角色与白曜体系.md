...
乌娜不是一套脱离游戏职业系统的自定义角色。`Career/wuna.csv` 声明标准 Career，游戏创建职业技能时执行 `SkillScript`，再由 `WunaScripts` 把职业被动和两个技能接入 SunExp 的战斗生命周期。
...
| --- | --- |
| Career 本地 id | `wuna` |
| 运行时完整 id | `SunExp_wuna_wuna` |
| 理智上限 | 70 |
| 职业脚本 | `CS.SunExp.Dll.Scripting.WunaScripts.InitCareer(self);` |
| 技能 1 | `SunExp_wuna_wuna_white_sun_prayer`，白曜圣祷，CD 5 |
| 技能 2 | `SunExp_wuna_wuna_grave_song`，圣庭墓曲，CD 4 |
| 职业技能卡 | `*wuna_white_sun_prayer`、`*wuna_grave_song` |
| 内部授冕牌 | `*wuna_coronation_token` |
| 动画库 | `Mods/SunExp/ModResource/AnimationLib/WuNa` |
...
主动技能仍走原生 `SkillItem.TrueUse`。反编译确认该方法先执行可用性检查，再设置 Target、运行 `UseScript`，随后处理语音、`InitScript`、数据刷新、Buff 刷新和 `FightUI` 行动动画。SunExp 没有绕开这条技能使用主链。
...
监听使用 SunExp 生命周期路由与 `EventDispose.OnFightEnd` 清理，避免职业切换或下一场战斗继续收到旧事件。
...
随后 `SunExpCardTagService.RequestBurnoutAndWhiteRadianceForFriendlyHands` 为己方全体当前手牌添加焚毁和白曜，并做去重。白曜的实际结算不在 CSV 的 `CardScript` 中，而由 `SpecialTagRuntime` 在真实行动前后识别。
...
`WunaRoundRadianceState` 以所有者为键记录“当前回合是否已触发”。它优先读取宿主 `FightManager` 的回合标识，无法取得时才使用 SunExp 的局部回合计数。当前实现满足以下条件时获得 1 日耀：

- 变化对象是敌方；
- 灼烧确实上升；
- 当前回合尚未触发。

这里存在一项需要维护者知晓的“文本与实现差异”：职业文本写的是“当乌娜使敌方获得灼烧”，但当前代码比较的是敌方灼烧总量前后值，没有调用 Buff 归因引擎判断施加者。因此，只要乌娜职业处于激活状态，其他来源在该回合首次提高敌方灼烧也可能触发日耀。除非后续接入明确的灼烧来源归因，文档不把它描述成已实现的施加者过滤。
...
该效果受性能设置 `SunExpWunaOrbitFireEnabled` 控制，默认关闭；启用后仍对几何刷新节流。视觉缺失或性能模式关闭不得影响职业机制。
...
- 数据：`SunExp/Data/Career/wuna.csv`、`Data/Card/wuna.csv`、`Text/Career/wuna.csv`。
- 脚本：`SunExp-Dev/Scripting/WunaScripts.cs`。
...
- 网络：`RpcEmberAdventureStateCommit.cs` 与 `SunExpNetworkRuntime`。
- 资源：`SunExp/audio.registry.json`、`visual.registry.json`、`SharedResources/cg.registry.json`、`SharedResources/package.json`。
