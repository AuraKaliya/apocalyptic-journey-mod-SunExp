...
| --- | --- |
| Career 本地 id | `loneer` |
| 运行时完整 id | `SunExp_loneer_loneer` |
| 理智上限 | 72 |
| 职业脚本 | `CS.SunExp.Dll.Scripting.LoneerScripts.InitCareer(self);` |
| 主动技能 | `SunExp_loneer_loneer_morning_star_prayer`，晨星祈愿，CD 2 |
| 职业技能卡 | `*loneer_morning_star_prayer` |
| 动画库 | `Mods/SunExp/ModResource/AnimationLib/Loneer` |
...
- 抽到白石：按袋中剩余黑石数量获得星辉，并触发自然晨星；
- 袋空后：按当前黑石上限重新放入黑石，再放入 1 白石并洗袋。
...
完成谱句还会通知使魔祝福扩展点。谱句的业务归属仍在 SunExp Mechanics，共享层只承担事件和调度。
...
余音可在费用不足时替代法力支付。运行时还在玩家行动期间为一次实际新增的正面或负面 Buff 提供 +1 并消费余音。

需要区分它与星辰祝福的回滚能力：星辰祝福有显式费用覆写存储和失败退款；余音的费用替代在 before-use 阶段直接调整一次性费用并消费层数，当前代码没有对“未观察到 Action”的余音建立对称退款记录。宿主通常只在真实使用链进入该 Hook，但维护 Hook 顺序或兼容新版本时必须专门回归拒绝出牌场景。

## 9. 晨星封印与实际费用

`morning_star_seal` 在 EnchTag CSV 中没有脚本体。`StarScoreRuntime` 与行动路由在卡牌真正执行后，按实际支付费用获得等量星辰祝福。

实际费用可能受到星辰祝福、余音、宿主其他减费和卡牌临时覆写影响，因此不能在拖拽开始时用原始 Cost 直接结算。运行时使用卡牌签名和 pending 栈关联 before/use/action/after，并在取消与销毁路径清理。
...
HUD 创建失败时使用下一帧有限重试，不在 `Update` 中无限查找。槽位点亮效果来自 `visual.registry.json` 中的 `sunexp.star_score_hud` Shader 与 `sunexp.star_score_hud.lit_slot` 材质。
...
- 数据：`SunExp/Data/Career/loneer.csv`、`Data/Card/loneer.csv`、`Data/Card/sunexp.csv`、`Data/Buff/sunexp.csv`。
...
- 资源：`SunExp/visual.registry.json`、`SharedResources/cg.registry.json`、`SharedResources/package.json`。
