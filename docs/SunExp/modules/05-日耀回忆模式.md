...
`SolarMemoryStarterDeckRuntime` 从启动时选中的卡包建立候选，要求最终正好 11 张。它通过 StarterDeck Arbiter 声明 `SunExp.SolarMemory` 所有权，防止其他模式或 MOD 同时覆盖开局牌组。
...
`SolarMemoryContentIsolationRuntime` 在非日耀模式下扫描生成树、选择参数和当前节点，把误入的专属 Map/Event 替换为同类型普通节点。它不会直接改写全局 Map 配置行。
...
`Fight_Win.ResetStates` 后读取 `FightManager.level`：
...
前奏完成后设置 pending 标记。主机在地图运行时可用时：
...
宿主 `NormalMapManager.ReadyToChangeMap` 在 Level >= 32 时显示 `GameExitUI`。SunExp 在第三层完成后把 Level 路由到 32，并由 `SolarMemorySettlementPresenter` 关闭地图/事件 UI 后打开原生结算。

旧存档若已落在旧终章等级 30 以上，会在 `MapItemInit` 前转回当前结算路径。逃跑和失败会关闭整备/对话临时 UI、清除隐藏 Boss pending，并修复 currentNode，但不伪造胜利结算。
...
| Mechanics | 路线节点池、节点应用、剧情门控、终局状态 |
| Hooks/UI | 模式入口、整备窗口、地图改写、Boss 分支、战斗/奖励与结算 |
...
| `NormalMapManager.RandomGenerate/GeneratrMap` | 捕获并改写地图池 | 反编译确认原生生成顺序 |
| `NormalMapManager.MapItemInit` | 固定 6 槽及视觉锁 | 反编译确认节点、SendNode、ReadyToSelect 顺序 |
| `MapSelectUI.ReadyToSelect/SendNode/ShowMap` | 可选牌、数组发送和客户端重建修复 | 反编译确认 |
| `MapManager.CmdSelectMap/TargetUpdateMap/RpcUpdateMap/RpcNextMap` | 修复权威数组和 currentNode | 反编译确认网络入口 |
| `Fight_Win.ResetStates` | Boss 胜利分支 | 代码和反编译确认 |
| `Fight_Escape.ResetStates`、`Fight_Loss.Init` | 异常退出清理 | 代码和反编译确认 |
| `NormalMapManager.ReadyToChangeMap` | 第三层完成后转原生结算 | 反编译确认 Level >= 32 行为 |
| `GameExitUI` | 最终结算界面 | 反编译确认类型，代码确认调用 |

## 13. 性能、兼容与已知限制

- 固定节点规格按层缓存，配置表查询走 `SunExpConfigIndex`。
...
主要入口：`SolarMemoryRunLauncher.cs`、`SolarMemoryPreparationRuntime.cs`、`SolarMemorySetupFlowRuntime.cs`、`SolarMemoryStarterDeckRuntime.cs`、`SolarMemoryBlessingPickerRuntime.cs`、`SolarMemoryModeRuntime.cs`、`SolarMemoryMapNodePoolFactory.cs`、`SolarMemoryContentIsolationRuntime.cs`、`SolarMemoryFlowApi.cs`、`RpcSolarMemoryRoleCommit.cs`。
