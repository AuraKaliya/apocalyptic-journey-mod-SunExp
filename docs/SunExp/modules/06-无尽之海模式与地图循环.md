...
地图仍使用宿主 `MapManager.CmdSelectMap/TargetUpdateMap/RpcUpdateMap/RpcNextMap`。SunExp 同时增加协议版本 2 的 `EndlessSeaStateSnapshot`，同步：
...
- 当前 run 没有自然胜利终点；`Ended` 主要用于存档管理和未来主动结束路径。
- 主动撤离、阶段结算和海域主题轮换尚未实施；已确认的候选方案单独记录在[无尽之渊阶段结算与海域轮换方案](../design/01-无尽之渊阶段结算与海域轮换方案.md)，不能把该设计稿当作当前行为。
...
主要入口：`EndlessSeaRunLauncher.cs`、`EndlessSeaRunStateStore.cs`、`EndlessSeaIntroBoardRuntime.cs`、`EndlessSeaStarterDeckCatalog.cs`、`EndlessSeaFloorPlanner.cs`、`EndlessSeaMapBuilder.cs`、`EndlessSeaNodePoolService.cs`、`EndlessSeaModeRuntime.cs`、`EndlessSeaCombatRuntime.cs`、`EndlessSeaRewardRuntime.cs`、`EndlessSeaCardAffixRuntime.cs`、`EndlessSeaNetworkSync.cs`。

重点回归：新建/续玩/重开、15 张主题卡组、六槽与八张节点牌、楼层 6->7、Boss 池、原生 Level 重置、奖励独占、baseline 不焚毁、新卡自动焚毁、客户端晚加入与快照修复。
