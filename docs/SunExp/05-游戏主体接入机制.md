...
```mermaid
flowchart LR
    Sun["SunExp"] --> A["MOD 发现与 Entry.dll 初始化"]
...
`SunExp.Dll.Entry.Initialize(ModConfig)` 是 SunExp 的 `[ModInitialize]` 方法。它不由 SunExp 自己主动调用，而是由游戏 `ModConfig.Setup` 反射调用。

接入失败的主要边界：

- `Entry.dll` 不存在时，游戏仍可能加载 Data/Text，但 C# 脚本不可用；
- Assembly.LoadFrom 或初始化方法抛出未处理异常时，`ModConfig.Setup` 返回失败；
- SunExp 内部具名 RunStep 会吞下单步异常并继续，因此日志是判断降级模块的必要证据。
...
因此 SunExp 不是在 C# 初始化时“手动注册每张卡”。大部分内容先作为标准游戏 DataConfig 进入官方表，C# 只通过脚本列和运行时 Hook扩展其行为。
...
SunExp 的 `Entry.RegisterLuaVisibleAssembly` 把 `SunExp.Aura` 加入 XLua translator assembly list，然后 `xlua.import_type` 校验主要 Scripting 类型。
...
SunExp 不把这些裸对象操作散布到 CSV，而是由 Scripting 把请求交给 GameApi。
...
这解释了 SunExp Hook 字符串为什么通常是 `Type.Method`，而不是程序集限定名或完整签名。

### 6.2 SunExp 注册路径

```text
Runtime.Initialize(modConfig)
-> SunExpHookRegistry 或 AuraSharedHooks
-> modConfig.AddMethodHookBefore/After("Type.Method", callback)
-> ModHookRegistry.AddBefore/AddAfter
-> Modifiable.OnEntry/OnSuccess
```

`AuraSharedHooks` 增加参数检查、日志、安全调用和 routed subscription。`SunExpHookRegistry` 再增加 owner 前缀。

### 6.3 属性 Hook

`ModConfig.Setup` 还会扫描 Entry.dll 中带 `HookBeforeAttribute` 或 `HookAfterAttribute` 的静态方法。`SpecialTagRuntime` 当前使用这一机制接入：

- `Fight_Start.Init` after；
- `CommonCardItem.TrueUse` before；
- `AttackCardItem.TrueUse` before。

属性 Hook 适合少量固定入口；大量可组合 Runtime 使用动态注册和 routed hook。

### 6.4 Hook 能与不能做什么

当前 `ModHookContext` 不提供可依赖的强类型返回值修改契约，主要暴露 Target 和 Arguments。SunExp 的 Hook 应以观察、预处理参数关联状态、补充流程和成功后的同步/表现为主，不能假设任意 before hook 都能取消原方法或替换返回值。

## 7. EventCenter 与脚本事件

游戏 `Witch.Core.EventCenter` 提供按字符串事件名注册和触发的生命周期通道。`ScriptExecutor.AddEvent/AddTempEvent` 也建立在游戏事件系统之上。

SunExp 通过 `ScriptEventApi` 集中处理：
...
高价值宿主边界包括：`UIManager.CloseUI`、`UIBase.Close`、`GameEntryUI.Init` 和 Fight Win/Loss/Escape。`SunExpUiLifecycleRuntime` 在这些边界关闭临时 UI。

### 9.2 射线与清理

游戏原生 upper canvas 会把残留子对象视为活动 UI。SunExp/Aura 的安全策略是：
...
SunExp 私有资源通常通过游戏 ResourceLoader/Unity 资源 API 解析，重复读取统一经过 `SunExpResourceCache`。VisualBundle、Shader 和 Material 由 `Hooks/Visual` 的缓存与加载器管理。

共享 CG、Skin 和 Audio 资源先由 Aura 包安装和注册，再由领域运行时按 `Shared:` 或注册 identity 解析。工具消费者读取协议声明，不直接扫描 SunExp 私有路径。
...
这使 Entry.dll 中的 SunExp 派生命令可以走游戏已有多态命令通道。

### 11.2 SunExp 权限补强

宿主 `CmdExecute()` 本身不替 SunExp 判定“谁有权改变某个 SunExp 状态”。SunExp 使用 `SunExpRpcAuthorityRuntime`：
...
| SunExp 接入面 | 游戏程序集 | 反编译类型/方法 | 方式 | 证据状态 |
| --- | --- | --- | --- | --- |
...
| 症状 | 优先检查 |
| --- | --- |
| CSV 提示 `CS.SunExp...` 不存在 | Entry.dll 是否加载、XLua assembly registration 日志、Scripting 类型名 |
