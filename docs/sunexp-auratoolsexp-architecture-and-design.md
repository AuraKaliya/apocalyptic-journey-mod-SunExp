...
- 一回合只生成一次 `CompanionIntentPlan` 快照。计划固定意图、数值、消耗、目标
  顺序、冷却就绪回合和预告威胁；执行阶段不得重新抽取意图或重新选择新目标。
- 魔力不足或没有合法意图时必须生成 `system.wait`，等待不会消耗魔力、进入冷却
  或制造预告威胁；回合结束仍按规则恢复魔力。
- 冷却存储为绝对 `ReadyOnTurn`，不得在多个回调中递减同一个剩余回合值。
- 意图注册表使用 schema 版本和稳定摘要；未知类型、未知效果或缺少 EnemyCard 的
  配置在加载阶段拒绝，联机双方摘要不一致时拒绝召唤请求。
