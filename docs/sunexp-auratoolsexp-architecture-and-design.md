...
## 11. Companion Projection 约束

- 服务端或单机实例是投影生命周期、BUFF、威胁、魔力、冷却和意图的唯一权威；
  客户端只应用带协议版本、战斗代次和状态修订号的完整快照。
- 投影身份必须显式携带 `OwnerPlayerId`、`OwnerStatusId`、`Faction=Friendly` 和
  `EntityKind=Companion`，不得再用本机角色 ID 推断归属或阵营。
- 使用投影牌时先捕获使用者当前正面/负面 BUFF，再逐项应用到投影；单个不兼容
  BUFF 通过 Direct/Adapter/Reject 策略隔离，不能中断整次召唤。
- BUFF 水合完成并统一刷新状态后，才计算基础威胁和首个意图。基础威胁限制在
  80–120，预告与近期威胁叠加后的最终权重限制在 80–200。
- 一回合只生成一次 `CompanionIntentPlan` 快照。计划通过 `ResolvedEffects` 固定
  Handler、数值、段数、BUFF、目标顺序、消耗、冷却就绪回合和预告威胁；执行
  阶段不得重新读取配置、重新抽取意图或重新选择新目标。
- 魔力不足或没有合法意图时必须生成 `system.wait`，等待不会消耗魔力、进入冷却
  或制造预告威胁；回合结束仍按规则恢复魔力。
- 冷却存储为绝对 `ReadyOnTurn`，不得在多个回调中递减同一个剩余回合值。
- schema v3 意图注册表使用完整规范化配置摘要；未知类型、Handler、目标策略或
  缺少 EnemyCard 的配置在加载阶段拒绝，联机双方摘要不一致时拒绝召唤请求。
- 实际效果只能通过 `CompanionIntentHandlerRegistry` 执行；未知 Handler 必须跳过
  并记录错误，禁止退化为默认伤害。目标选择通过独立 Target Policy 解析。
- 攻击／防御倾向使用 Profile 中独立的 60／40 权重，不能按池内条目数量求和；
  选定倾向后只进行一次意图加权，紧急治疗可以修正防御倾向。
- 战斗常驻 HUD 必须挂在 `FightUI` 内的 `BattleHudHost`，与角色战斗 HUD 同域；
  禁止挂到全局最上层 Canvas，也不得通过 `SetAsLastSibling` 压过选牌或弃牌界面。
