...
| `Mechanics` | 131 | SunExp 业务服务、状态、规则、注册表和规划器 |
...
文件数量用于判断维护规模，不代表层级优先级。`Hooks` 与 `Mechanics` 文件较多，是因为大型模式和表现运行时也在同一 SunExp 程序集中。
...
- 所有 CSV `CS.SunExp.Dll.*` 调用必须以 `Scripting.` 开头；
- Scripting 不得 `using SunExp.Dll.Hooks`；
- Scripting 不得直接使用 hook-owned `SunExpFrameScheduler`；
- Scripting 不得裸调用 `AddEvent`/`AddTempEvent`，应经 `ScriptEventApi` 或 `ExecutorApi`；
- Dialogue CSV 不得直接调用 SunExp C#。
...
| 配置访问 | `CardConfigApi`、`GameCompatibilityApi`、`SunExpResourceCache` | DataConfig、签名漂移、资源缓存 |
...
`Features/SkillCg/SunExpSkillCgRuntime` 是当前唯一 Feature。它由 Entry 初始化，不是 CSV 公共入口，负责把 SunExp 内容触发转换为共享 Skill CG 请求。
...
- `SunExpHookTargets` 集中保存高频宿主目标字符串；
- `SunExpHookRegistry` 包装 `AuraSharedHooks` 并默认 safe invoke；
- Aura routed hooks 让同一宿主目标共享一个注册分发器；
- `SpecialTagRuntime` 展示属性式 `[HookBefore]/[HookAfter]` 入口。

新增高频目标应进入 `SunExpHookTargets`。一次性、强局部目标可以保留在对应 Runtime，但仍需进入宿主映射表。
...
- `SunExpNetworkRuntime`，发送和本地/联机通道路由；
- `SunExpRpcAuthorityRuntime`，绑定服务器接收上下文中的真实 sender；
- 场地、无尽之海状态同步；
- 日耀回忆角色提交；
- 投影、心变、百变、冒险状态和深渊表现命令。

改变共享进度或权威状态的命令实现 `ISunExpServerBoundRpcCommand`。payload 中的 role/player/reporter 字段只能作为待验证声明，不能作为权限来源。

## 10. Infrastructure：低层稳定面

当前基础设施包括：

- `SunExpIds`、`SunExpFieldId`、`SunExpHardTagIds`；
- `SunExpLog`；
- `SunExpPerformanceSettings`、`SunExpPerformanceCounters`；
- `SunExpFrameDispatcher`、`SunExpFrameStepRunner`、`SunExpLifecycleStepRunner`；
- `SunExpDirtyState`、字典辅助和 UI 诊断。

Infrastructure 可以包装 Aura 核心设施，但不应包含卡牌、剧情、模式奖励等业务策略。当前 `SunExpCombatCardUiDiagnostics` 读取 GameApi 是一个诊断型例外，新基础工具不应据此普遍反向依赖业务适配层。

## 11. 架构门禁

`tools/Test-SunExpArchitecture.ps1` 既检查文件存在，也检查关键依赖规则和实现锚点，包括：

- handler registry 不退化；
- Scripting 不直连 Hooks 或裸事件；
- ResourceLoader 读取集中到 `SunExpResourceCache`；
- Hook/Mechanics 表扫描集中到 `SunExpConfigIndex`；
- 模式启动、准备、UI teardown、pool、缓存和性能设置保持在指定边界；
- CSV C# 目标只进入 Scripting；
- Dialogue CSV 不直连 C#。

`tools/Test-SunExpCSharp.ps1` 使用聚焦测试工程编译选定源码和 stubs，验证关键业务与兼容行为。它不是完整游戏集成测试，因此仍需正式构建和内容验证。
...
被多个 MOD 需要且可移除 SunExp 语义？ -> Aura 共享候选
```
