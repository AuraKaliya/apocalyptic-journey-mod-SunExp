...
图鉴卡按原生 `ICard.SetCardStyle` 的 Image/MeshRenderer 分支读取 `Front/FrontBack` 的实际纹理、UV 与可见边界，使用 Canvas 相机投影处理网格卡框的深度偏移；不更换原生材质。`TerriasUiSprites` 缓存生成的纯色 alpha 光圈，GPU 读回兼容不可读贴图。面板关闭和卡片回收清空选中状态，光圈遵守列表 Mask 裁剪。
...
轮廓数值测试为 `Terrias-Dev.Tests/SilhouetteGlowTests.cs`；Unity 验证入口为 `tools/Test-TerriasAbyssSelectionUnity.ps1 -UnityPath <Unity 6000.0.46f1/Editor/Unity.exe>`。该检查使用真实两张素材及带翻转 UV、深度偏移的合成原生 Mesh 卡框，验证 GPU 像素变化、取消后恢复、透明点击区域、裁剪、缓存和复用；它不代替完整游戏进程内的界面验收。截图和带源码/素材指纹的结果存放在 `output/abyss-selection-unity`。
