...
`Terrias/endless_abyss.config.json` 当前关键值：
...
进化特征池由 `endless_abyss.evolution_traits.registry.json` 声明，当前包含原生高级 SpecialBuff 和 Terrias 镜阵特性。配置加载失败时使用内置规范化 fallback。

## 3. 注视等级与选择数量

`EndlessAbyssGazeService` 将注视保存在 `Terrias_EndlessAbyssGazeLevel`。震荡必选数量依据 `choiceStep=3` 增长并钳制到 1-3；进入第 7 层时最低提升到 5。
...
战斗临时诅咒带 `TerriasTemporaryAbyssCurse` marker，加入抽牌堆后随机插入，并在战斗边界清理。永久诅咒通过 `PlayerApi.TryAddCardToDeck` 写入角色牌组。添加诅咒时抑制注视的“获得卡牌”计数，防止诅咒自身递归制造更多压力。
...
从第 2 层起，里程碑按“玩家 + 楼层”记录领取结果。下列四类奖励组成候选池，每次抽取三个不同选项，选中后点击确定：
...
异次元奖励池优先从配置的卡包动态生成；为空时退回 `polymorph`、`witch_projection`、`heart_change`。卡牌实际加入角色的装备卡组或备用卡列表时附加焚毁，并随角色存档保存。

个人里程碑结果包含 player id/role id、floor 和结果；新结果用一条 receipt 同时记录奖励内容与楼层领取。读取时仍识别旧版本单独保存的楼层 key。卡牌奖励严格按实例 ID 查找；目标失效时保留奖励并要求重选，不再回退到同名牌或第一个候选。卡牌变更在保存/领取记录提交失败时恢复原状态。

### 三卡选择与刷新（2026-09-10）

震荡和里程碑共用 `EndlessAbyssChoiceView`，各自使用 `ModResource/Images/UI/无尽之渊UI` 下的素材。按 1920×1080 参考画面设置卡片高度 600，保持素材比例：里程碑 400×600，震荡 450×600。窗口随 Canvas 尺寸等比适配。里程碑名称位于中部横栏，震荡名称位于上半圆形区域，正文分别占用素材下方的深色区域。

每个卡位在本次面板中可免费刷新一次，不扣金币。刷新只替换对应卡位，并清除该卡位的选择；其他卡位和选择保留。候选不与同屏选项重复，无其他可用选项时不消耗次数。`EndlessAbyssChoiceState` 保存三个候选、各位刷新使用状态及已选项，`EndlessAbyssChoiceStore` 按玩家及当前请求/楼层持久化。返回或读档不重新发放刷新次数。震荡仍按注视要求选择 1～3 项；里程碑选 1 项。最终提交检查当前候选和选择状态。

净化及绝灭目标通过图鉴的 `DictionaryShowItem` 模板展示卡面、名称、效果、费用及标签，使用实际实例 Vars 的展示副本，保留实例 ID 以显示其附着。副本禁用 InitScript，浏览不会执行卡牌效果或重置实际标签。卡牌按实例去重，同名不同实例分别保留；选中后再次确定才领取。返回保留主面板的候选与刷新次数。关闭时清理展示副本和共享视觉租约，卡片单元交回 UI 池。

选中反馈使用 `EndlessAbyssSelectionGlow`：沿素材透明通道的外轮廓显示约 3 UI 单位的金色亮边与 9 单位的柔光，不使用矩形包围框。外部洪泛和距离场排除封闭的内部镂空，光圈不会染色卡面或正文。悬停降低亮度，已选状态在鼠标移开后保留。图鉴卡移除单元底色，透明 Image 仅承担点击；光圈独立且不拦截射线。

图鉴卡按原生 `ICard.SetCardStyle` 的 Image/MeshRenderer 分支读取 `Front/FrontBack` 的实际纹理、UV 与可见边界，使用 Canvas 相机投影处理网格卡框的深度偏移；不更换原生材质。Image 分支与 Unity `GetDrawingDimensions` 一致：先等比适配，再用 `DataUtility.GetPadding(sprite)` 扣除透明留白，最后与 `GetOuterUV` 的裁剪纹理区域配对。原生卡框的 Sprite 是 384×384，实际纹理矩形约 219×338；忽略留白会把光圈放大到承载方框。该边界修复于 2026-09-12 加入真实原生纹理的紧密打包回归，并直接与 Unity 生成的 Image 网格比较。`TerriasUiSprites` 缓存生成的纯色 alpha 光圈，GPU 读回兼容不可读贴图。面板关闭和卡片回收清空选中状态，光圈遵守列表 Mask 裁剪。

### 永久焚毁与旧档迁移

`EndlessSeaCardAffixRuntime` 订阅当前角色两份 `ObservableCollection<DataConfig>` 的新增事件；角色或集合替换时解除旧订阅并绑定新集合。监听持续覆盖战后领奖，展示 Init/DataUpdate 不再写入焚毁。开局卡组写入抑制范围及开局实例身份保证初始卡牌不被奖励规则影响。

净化删除实例的有效 `Vars.Tag` 中的 Burnout，并记录净化对应的实例 ID。显式空 Tag 是有效覆盖，不回退到基础表。自动附着跳过该净化实例；新获得的副本具有不同实例 ID，不继承旧副本的净化或开局豁免。

旧档在角色/地图/开战或候选读取边界进行一次迁移：依据开局基线补齐可确认的奖励卡焚毁，依据保留的旧净化 receipt 恢复精确实例的净化标记。没有开局基线或确切记录的未知卡牌不作推断修改。迁移完成后保存版本标记，后续新增奖励由持有列表入口处理。

回归入口为 `Terrias-Dev.Tests/EndlessAbyssChoiceTests.cs`。实机验收需覆盖：领奖后立即查看标签与净化候选、同名副本去重、净化后跨战斗和读档、三个刷新位独立耗尽及读档保持、不同注视等级的确认数量、720p/1080p/宽屏布局、卡牌网格滚动与退出后其他 UI 的点击。

轮廓数值测试为 `Terrias-Dev.Tests/SilhouetteGlowTests.cs`；Unity 验证入口为 `tools/Test-TerriasAbyssSelectionUnity.ps1 -UnityPath <Unity 6000.0.46f1/Editor/Unity.exe> -GameDataDirectory <游戏 Data 目录>`。首次提取原生卡框后可复用与仓库 Managed 指纹一致的缓存。四组检查使用真实两张大卡素材、真实原生卡框纹理的紧密打包 Sprite，以及带翻转 UV、深度偏移的合成 Mesh 卡框，验证 GPU 像素变化、透明留白与等比缩放、取消后恢复、点击、裁剪、缓存和复用；它不代替完整游戏进程内的界面验收。截图和带源码/素材/原生资产指纹的结果存放在 `output/abyss-selection-unity`。
...
两个 resolution RPC 都按主机到客户端广播的方向定义，不接受客户端自报决策作为权威结果。其中震荡 RPC 已被当前结算路径调用；里程碑 RPC 目前只是已实现但未由面板启用的路径。真正的主机身份和房间成员校验发生在状态发布/请求及 Terrias 网络发送边界。
