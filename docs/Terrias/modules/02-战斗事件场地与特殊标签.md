...
| 月之领域 | `moon_domain` | `Terrias_terrias_moon_domain` | 1 | 月反应替换策略 |

启动时 registry 从 Buff DataConfig 读取实际 UpperBound、名称和描述；64x64 HUD 图标由场地定义独立注册。轮回花庭的实际第 5 层沿用第 4 档图标，但 HUD 数字和效果仍按 5 层处理。

场地快照同时驱动专属远景、场景纹样/粒子和手牌/UI 背光。场地替换交叉淡化，同类叠层更新视觉强度，战斗进入结算时清理。表现不改写原生背景身份、平台、BGM、卡面材质和后处理。配置、资源及验证入口见[场地视觉表现](D:/Project/Apocalypic-journey-mods-creator/ModExp/docs/Terrias/design/field-presentation-runtime.md)。
