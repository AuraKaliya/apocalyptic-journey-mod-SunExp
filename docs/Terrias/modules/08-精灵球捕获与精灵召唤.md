...
| 召唤位置 | 固定附着在拥有者右上角；与投影正式友方阵位独立，可同时存在 |
...
投影和精灵不再共享位置。投影作为正式友方 `Partner` 进入右侧阵位；精灵也具有独立 `Partner` 身份和意图池，但表现固定附着在拥有者右上角。同一拥有者可以同时持有一个投影和一个精灵；同类精灵仍按原轮换规则替换。

召唤链为：

1. `SpiritSummonService.CanSummon` 检查战斗状态、动态快照和 `Idle` 资源，不再检查投影占位。
2. 从 `Model/player` 创建与投影相同结构的表现宿主。
3. `SpiritOtherObj` 建立内部 `StatusManager`、行动卡和 `CompanionBattleState`。
4. `SpiritAttachmentPresenter` 复用 `ProjectionVisualProxy`，把敌人待机图挂到玩家右上角；右侧绘制自下而上填充的竖向生命条，常驻 Buff 列表被隐藏。
5. `ProjectionTurnCoordinator` 将精灵直接插入宿主原生 Partner 队列；鼠标悬停精灵或生命条时，由原生状态 hover 面板展示完整状态和 Buff。
...
精灵是独立正式友方目标，拥有自己的生命、属性、Buff、资源和意图；拥有者只负责归属和生命周期。精灵伤害仍按统计规则归属拥有者。
