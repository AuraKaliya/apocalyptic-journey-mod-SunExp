...
乌娜的主动技能语音按 Career 配置序号绑定：`Skill1=白曜圣祷`、`Skill2=圣庭墓曲`。白曜圣祷脚本不直接播放“愿白昼永不落幕”；共享音频层在技能事务 `Committed` 后解析 `SkillDataId -> SkillSlot 1`，再由 Terrias Provider 决定语音资源与联机表现同步。
...
- 必需表现资源：Terrias `visual.registry.json` 与乌娜轨道火/星谱反馈资源。
- 乌娜可选语音和技能/卡牌/美餐 CG 由 Terrias `SharedResources` 携带，AuraToolsExp 发现并配置；替换皮肤和卡牌主题仍位于 AuraToolsExp。播放与联机由 Aura 共享层负责。
