...
日耀回忆是 Terrias 的角色专属大型模式，但当前实现没有创建一套独立的宿主 `IModeManager`。它创建 `modeType = Normal` 的标准存档，并用 `Terrias_SolarMemoryMode = 1` 标识专属路线，再在普通模式地图、战斗和结算流程上做受控改写。

这条边界很重要：

- 宿主仍负责角色、卡牌、地图 UI、战斗与存档基本生命周期；
- Terrias 负责整备状态机、专属内容隔离、固定节点、Boss 分支和异常修复；
...
| Map id | Level id | 位置 |
| --- | --- | --- |
| `solar_memory_boss_orbit_mirror_array` | `Terrias_terrias_level_orbit_mirror_array` | 第二层固定首领 |
| `solar_memory_boss_second_sun_last_day` | `Terrias_terrias_level_second_sun_last_day` | 第三层倒数第二槽 |
| `solar_memory_boss_saint_wuna` | `Terrias_terrias_level_saint_wuna` | 隐藏终局 |

这些 Map 行的 `Rarity=7` 只是数据标记，真正隔离还依靠 `TerriasIds` 判定、模式节点工厂和非日耀模式清洗器。
...
`SolarMemoryDeckIsolationRuntime` 负责初始/当前卡包选择、CardPackCheck 过滤、事件卡识别、角色牌组清洗、备用池清空和原生牌组窗口入口。`SolarMemoryStarterDeckRuntime` 使用该边界提供的卡包与分类结果建立候选，要求最终正好 11 张，并通过 StarterDeck Arbiter 声明 `Terrias.SolarMemory` 所有权，防止其他模式或 MOD 同时覆盖开局牌组。
...
整备完成后才调用 `SolarMemoryRoleCommitApplication.SubmitFinal`。提交包含一次性 token：

- 单机/主机在服务端本地应用；
- 客户端通过 `RpcSolarMemoryRoleCommit` 提交；
- 服务端校验真实 sender、房间成员、`role.Id == sender.PlayerId`、模式标记和完成状态；
- 角色与提交回执在同一权威存档中原子保存，重复请求只返回回执；
- 客户端保留冻结的待提交内容，超时表示结果未知，重连后先查询；
- 明确拒绝才撤销完成标记；旧房间回执不能完成新冒险整备。

## 6. Aura Journey 路线

`SolarMemoryJourneyApi` 以 owner `Terrias` 注册 Journey `Terrias:Terrias.SolarMemory`，包含 preparation、route 和 boss 三类节点。路线图共三层：每层宿主长度 6，默认段长度 6、可选段长度 8。
...
地图运行采用四段边界：`SolarMemoryMapLifecycleCoordinator` 负责地图生成、选择数组修复、客户端 `currentNode` 恢复和固定槽重投影时序；`SolarMemoryMapVisualRuntime` 只注册 DataUpdate、MapItemInit 和 ShowMap 表现 Hook；`SolarMemoryMapProjectionRuntime` 创建或复用固定槽 `MapItem`、链条、标题与地图卡纹理；`SolarMemoryModeRuntime` 仅保留模式组合初始化、整备窗口入口和模式状态查询。地图卡声明继续来自 `VisualRegistry`，资源读取经过 `TerriasResourceCache`。
...
- 固定节点规格由不可变的按层目录复用，配置表查询走 `TerriasConfigIndex`。
- 地图卡纹理通过 `VisualRegistry` 和 `TerriasResourceCache` 解析；重复 ShowMap 会复用带 `FixedSlotVisualState` 的 MapItem。
...
数据入口：`Terrias/Data/EventList/terrias.csv`、`Text/EventList/terrias.csv`、`Data/Map/terrias.csv`、`Text/Map/terrias.csv`、Enemy/Level/Dialogue 对应行。
