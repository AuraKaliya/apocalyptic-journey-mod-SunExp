...
`SaveInfo.modeType` 始终修复为宿主 `Normal`，真实模式身份由 `Terrias_EndlessSeaMode = 1` 和 run id 决定。旧 `TerriasEndlessSea` modeType 只用于迁移识别，不是当前地图管理器类型。
...
宿主反编译确认 `NormalMapManager.MapItemInit` 默认只从 `DefaultNode` 取起点和终点，并通过 `MapSelectUI.ReadyToSelect` 从 `SelectNode` 取 8 张节点牌。因此 Terrias 构造一个兼容投影：
...
这样既保持宿主 `MapItemInit` 不崩溃，也得到 Terrias 的六槽语义。
...
`BattleRewardsUI.Entry` 反编译确认最终会调用 `MapManager.TryChange`。Terrias 在 Entry 后排入深渊震荡并结算本源战后效果。
...
地图仍使用宿主 `MapManager.CmdSelectMap/TargetUpdateMap/RpcUpdateMap/RpcNextMap`。Terrias 同时增加协议版本 3 的 `EndlessSeaStateSnapshot`，同步：
...
联机结算屏障当前使用协议 v2。客户端在 `GameApp.ReturnToMenu` 真正清理网络前完成本地持久化并发送 sender 绑定、token 幂等的 `PlayerCommitted`；不再等到 `GameExitUI.OnDestroy` 后发送。房主的 15 秒等待与 2 秒强制提交宽限由 Terrias 运行时帧调度器推进，不依赖结算 UI 的 `Update`。结算 UI 只投影进度和提交用户意图；UI 被销毁或重建不会暂停房主状态机，房主始终最后关闭房间。

## 13. 宿主映射

| 宿主类型/方法 | Terrias 行为 | 证据 |
| --- | --- | --- |
...
主要入口：`EndlessSeaRunLauncher.cs`、`EndlessSeaRunStateStore.cs`、`EndlessSeaIntroBoardRuntime.cs`、`EndlessSeaStarterDeckCatalog.cs`、`EndlessSeaFloorPlanner.cs`、`EndlessSeaMapBuilder.cs`、`EndlessSeaNodePoolService.cs`、`EndlessSeaModeRuntime.cs`、`EndlessSeaCombatRuntime.cs`、`EndlessSeaRewardRuntime.cs`、`EndlessSeaCardAffixRuntime.cs`、`EndlessAbyssEvacuationRuntime.cs`、`EndlessAbyssEvacuationService.cs`、`EndlessAbyssSettlementBarrierRuntime.cs`、`RpcEndlessAbyssEvacuation.cs`、`RpcEndlessAbyssSettlementBarrier.cs`、`EndlessSeaNetworkSync.cs`。

重点回归：新建/续玩/重开、15 张主题卡组、六槽与八张节点牌、楼层 6->7、Boss 池、原生 Level 重置、奖励独占、baseline 不焚毁、新卡自动焚毁、全楼层主动撤离、待处理结算拦截、撤离结算恢复、客户端晚加入与快照修复，以及 2/4 人下客户端先提交、房主先确认、ACK 丢失、UI 销毁和重复 RPC。
