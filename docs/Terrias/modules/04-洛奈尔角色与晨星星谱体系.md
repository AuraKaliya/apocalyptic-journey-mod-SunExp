...
> 模块范围：洛奈尔职业、星石袋、奇迹时钟、指引牌、自然晨星/借来的奇迹、晨星祈愿、星辰序曲、星谱句式、星辰祝福、余音、诅咒与众生愿力系列和星谱 HUD。
...
| 公开晨星卡 | 原有 8 张星谱牌，加逆转术式、晨星：回光、恶兆转移、众生相、众生愿、众生渡、晨星：悲歌，共 15 张 |
...
15 张公开晨星卡由 `CardScripts` 转交 `MorningStarCardScripts`。原有 8 张围绕谱句编辑与复现；新增 7 张把有效 `Curse` 转化为可保留、焚毁、反转和供养的资源。CSV 只保留统一脚本入口，具体效果由晨星脚本、`MorningStarCurseService` 和 `StarScoreService` 完成。

### 7.4 诅咒、众生相与愿力

- 逆转术式按数据驱动反转表处理已知诅咒，未知 MOD 诅咒按等阶获得愿力与星辉；
- 晨星：回光同时快照抽牌、手牌和弃牌区，按运行时实例去重后焚毁；
- 众生渡只快照出牌后剩余手牌，焚毁完成后再抽牌和恢复魔能；
- 晨星：悲歌先固定实际生命流失和触发次数，再逐次走正常抽牌入口；
- 众生相从 7 个稳定祝福 id 的差集中随机，结果写入本地冒险角色表；众生愿按不同 id 去重计数；
- 黑日十字在 `StartRoundEnd` 统计手牌诅咒，在 `EndRound` 按愿力恢复且不消耗愿力。

遗忘者现归属晨星卡包，其现有战斗效果已从 `SolarBlessingService` 迁入 `MorningStarBlessingService`。本系列复用主体 `buff_VowPower` 与 Terrias `starlight`，不新建同义资源。
...
- 诅咒选择、卡区扫描、愿力和星辉结算只由本地使用者发起；众生相随机结果通过主体角色表同步，不增加平行 gameplay RPC；
...
- 机制：`LoneerMiracleService.cs`、`LoneerCombatState.cs`、`StarStonePouchService.cs`、`MiracleClockService.cs`、`StarScoreService.cs`、`MorningStarCurseCatalog.cs`、`MorningStarCurseService.cs`、`MorningStarCurseFormula.cs`。
- Hook/UI：`LoneerRuntime.cs`、`StarScoreRuntime.cs`、`StarScoreHudRuntime.cs`。
- 必需表现资源：`Terrias/visual.registry.json`；晨星技能/美餐 CG 由 Terrias `SharedResources` 携带并由 AuraToolsExp 发现配置，替换皮肤和卡框主题仍位于 AuraToolsExp。
- 反编译：`Witch/FightInit`、`Witch/SkillItem`、`CommonCardItem`、`AttackCardItem`、CardItem 取消/销毁和战斗 UI 类型。

修改本模块后至少回归：无候选指引牌、连续多次抽石、白石与时钟同时到达边界、UI 忙碌延迟、晨星祈愿黑石下限、时钟上限 6、全部谱句、星辉跨阈值、费用取消退款、诅咒三卡区去重、反转兜底、悲歌死亡中止、黑日十字高愿力恢复、众生相跨战斗保留、战斗结束清理和百变压制。
