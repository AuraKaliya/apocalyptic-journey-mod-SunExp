...
> 文档基线：2026-08-26
> 适用版本：Terrias `0.5.0`，游戏反编译参考 `v1.0.24831968`
...
| `Application` | 认证命令、用例事务、持久化/网络 port、提交后事件 | Unity UI、RPC 序列化、Mechanics 内部算法 |
| `Features` | 非 CSV 的独立功能运行时 | 取代 Gameplay Hook 总入口 |
| `Hooks` | 生命周期接入、运行时路由、UI 和视觉变更 | 成为 CSV 直接调用面 |
| `Network` | RPC DTO、sender 绑定、payload 限制和传输 | 存档、领域提交、UI 投影、从 payload 自行认定权限 |
...
依赖方向由 `tools/TerriasArchitectureGate` 的 Roslyn 语义模型执行。历史反向引用必须逐文件记录在锁定债务清单中，包含负责人和删除里程碑；未登记新边、陈旧例外、别名/全限定绕过和有效图中的依赖环都会使门禁失败。EndlessSea 已完成首个 Application 垂直切片，后续遗留项按 `architecture-layer-cutover-v2` 逐项清零。
...
| 语义架构门禁 | `tools/TerriasArchitectureGate/`、`tools/architecture-boundary-exceptions.json` |
