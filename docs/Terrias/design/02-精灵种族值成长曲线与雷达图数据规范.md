...
> 文档状态：schema 2 运行时实现说明
> 基线日期：2026-08-10
> 当前配置：`Terrias/spirit.growth.registry.json`，schema 2
> 数据范围：55 个游戏主体形态档案 + 3 个 Terrias 档案
...
| 派生层 | 当前四大本源、生命、攻击、护甲、雷达归一化值 | 查询时计算 | `SpiritGrowthService` / `SpiritGrowthQueryService` |
...
- schema 1 文件仍可由 `SpiritGrowthRegistry` 在内存中迁移。
- 旧档案自动获得 Terrias 稳定 `speciesId/profileId` 和 schema 2 默认曲线引用。
- 不回写旧配置文件；正式发布配置已经升级为 schema 2。
- 不受支持的 schema 或非法引用会整体回退到有类型默认值，并输出诊断。

schema 1 只保留加载兼容，不再作为生成工具的输出格式。
...
schema 2 已把物种值、等级曲线、资质抽取、资质曲线、经验曲线、战斗换算和雷达尺度统一纳入有类型配置。
...
图中同时显示当前个体固定资质、资质 60 标准曲线和资质 100 理论曲线。生命、攻击、护甲与累计经验曲线保留为后续扩展，不在本轮 UI 中提供。
...
当前只读查询服务集中生成 UI 与调试所需数据：

```text
SpiritGrowthQueryService
  Build(instance)
  BuildCurve(profile, aptitude)
```

当前实现按打开详情时即时生成 50 个采样点，不在帧循环中计算。收藏规模或比较功能扩大后，可按以下边界增加缓存：
...
| 责任 | 文件 |
| --- | --- |
| schema 2 配置 | `Terrias/spirit.growth.registry.json` |
...
| 统一雷达与曲线快照 | `Terrias-Dev/Mechanics/SpiritGrowthQueryService.cs` |
| 个体永久存储 | `Terrias-Dev/Mechanics/SpiritCollectionService.cs` |
| 雷达与成长曲线图形 | `Terrias-Dev/Hooks/Ui/SpiritGrowthGraphics.cs` |
| 详情与仓库 UI | `Terrias-Dev/Hooks/Ui/SpiritManagementPanel.cs` |
| 配置生成 | `tools/Generate-SpiritGrowthRegistry.ps1` |
| 内容校验 | `tools/Test-SpiritCapture.ps1` |

## 20. 本轮落地状态

1. **已完成：**schema 2、schema 1 内存迁移、有类型曲线和注册表 hash。
2. **已完成：**55 个游戏主体形态与 3 个 Terrias 档案进入运行时配置。
3. **已完成：**个体存档升级到 version 7，捕获时固定 `speciesId/profileId`、固有能力计划和四语展示快照。
4. **已完成：**统一查询快照、四轴雷达和当前/潜力精确值；仓库当前不展示成长曲线。
5. **已完成：**仓库筛选、等级/资质/阶级/捕获时间排序、虚拟化列表、归元和携带操作。
6. **已完成：**配置结构、预算、身份、雷达上限和运行时迁移测试。
7. **后续扩展：**双个体并排对比和批量管理；不在当前规格中恢复成长曲线。

UI、战斗换算和开发校验现在共享同一份 schema 2 配置与成长服务；图形组件只绘制查询快照，不持有第二套数值公式。
