...
3. **已完成：**个体存档升级到 version 3，捕获时固定 `speciesId/profileId`。
4. **已完成：**统一查询快照、四轴雷达、当前/潜力精确值和三条 1–50 级曲线。
5. **已完成：**仓库筛选、等级/资质/阶级/捕获时间排序、收藏、锁定和携带操作。
6. **已完成：**配置结构、预算、身份、雷达上限和运行时迁移测试。
7. **后续扩展：**双个体并排对比、生命/攻击/护甲曲线、批量管理和缓存。
