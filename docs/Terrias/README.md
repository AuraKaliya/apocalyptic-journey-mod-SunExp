...
5. [精灵仓库、背包与养成 UI 当前规格](design/07-精灵仓库背包UI与养成系统实现规格.md)
6. [精灵养成能力维护规则](design/08-精灵养成能力维护表.md)

设计目录只保留当前规格、维护规则和仍有效的数值依据。已落地的评审稿与实现流水账在结论并入模块文档后删除；历史通过 Git 和更新日志追溯。模块文档描述当前玩家可见行为。
