...
6. 玩家只选择背景、构图预设、动作强度和资源；不直接编辑座位坐标、ZIndex 或裁切数字。
...
默认美术按四张基础底图组织：开场、胜利、失败、结算。三种特殊胜利通过各自的前景、色彩和短动效覆盖胜利底图。四张基础底图都必须保留角色安全区和标题安全区。
...
玩家本地配置只保存与默认值不同的字段。恢复默认即删除当前场景覆盖，不复制默认完整对象。
...
静态背景、静态标题与动态角色层使用分离的 Canvas 批次。场景仍由一个共享 Overlay 生命周期所有，不允许每个事件创建自己的 Canvas。
...
1. 七个 AuraToolsExp 事件注册项从 `team-stage.v1` 切换到对应 v2 profile。
2. 单一 `DPS-CG.png` 不再作为所有场景的默认背景；保留时只作为明确场景的兼容资源。
3. 删除 `AuraCgTeamSceneLayout` 的 v1 固定排布实现及只验证正宽高的旧测试。
4. 删除设置页的姓名方块假预览，改为共享渲染器嵌入模式。
...
3. `SceneProfile`、四张基础背景和三种特殊胜利覆盖。
