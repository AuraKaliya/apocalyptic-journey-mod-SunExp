...
- 卡牌和遗物只在通过原生“世界推演”入口绑定了 `Witch:world-simulation` 运行身份时应用。`NormalMapManager` 只是宿主实现，不能作为模式身份；任何未声明身份或由其他 MOD 激活的自定义模式均失败关闭。旧版在非世界推演中写入的 AuraTools 遗物标记会按记录的精确 ID 单向清理。
