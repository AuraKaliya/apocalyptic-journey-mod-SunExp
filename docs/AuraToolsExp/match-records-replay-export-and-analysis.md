...
新记录以最终被原生受理的决策及其结果为主线。出牌、技能、结束回合和动作中的确认选择是决策；
免疫或抵消仍是有效结果。拖牌、持牌犹豫、取消、悬停与未确认选择不构成对局动作。原生入手或
重排被输入接管时在接管边界结束观察，防止把操作过程留在旧的入手/重排轨道中。确认选择后的
自动重排仍然记录，即使原生尚未执行选择 UI 的清理。

`confirmed-decisions.v1` 声明录制的输入等待见证。开始/结束等待和确认决策以不可变事件进入已有
journal；原始时刻与封存根保持不变。`ReplayDecisionTimelineV17` 为播放器生成派生的观看时间轴，
将可裁剪等待替换为一次 0.5 秒停顿；自动连锁不按底层事件逐个停顿。已观察到的动作、移动、命中、
特效与有限音效区间受到保护，包括晚到的观察事件。背景音乐和持久 HUD 不阻止等待压缩。
联机的 `OtherTurn` 记录本视角等待远端动作的区间，不声称识别远端玩家的私有输入状态；本地队列、
开放来源和采样排空后才能打开该区间，收到的有效动作及状态变化会关闭它，实际观察到的执行仍受保护。

事件、检查点、轨道偏移、音频起点/持续长度和视频回合定位共用同一映射。完全被裁掉的音频从
观看安排中省略，不能以零时长传给将零解释为“播放完整片段”的音频运行时。倍速作用于观看时钟。
旧 v17 记录缺少输入见证时仍由同一播放器按原始节奏播放，并显示“原始节奏（旧记录）”；不猜测
空白区间、不改写旧文档。联机录制协商要求新决策时间轴能力，避免向旧客户端发送其无法读取的新记录。
...
角色、相机、冲击和状态提交允许重叠；观看时间轴只压缩有见证且不覆盖必要表现的等待区间。
...
- 视频帧数、音频离线混音和回合导航使用播放器的同一观看安排，编码 profile 标识此节奏契约；
