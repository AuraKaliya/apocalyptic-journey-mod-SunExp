...
2026-08-05 又完成了运行时探测与分阶段资源自动调优：CPU/CUDA Python 环境、模拟并行、
推理 lane/batch、梯度分片、教师 microbatch 和检查点序列化均保留 `0/auto` 请求语义，
通过硬件实测与版本化缓存生成有效计划；教师数据预张量化、长度分桶、CUDA 预取与混合
精度已经落地。该调优只改变执行计划，不改变样本、优化器 step 或确定性语义。
同日完成第二轮训练调度改造：Transformer 教师支持跨轮权重热启动、CPU 间隔刷新和最终
Epoch，子进程进度/CPU/内存进入控制中心；最终验证改为固定窗口滚动调度和确定性顺序
提交；Replay 新增通用策略阶段分层，开发档增加自博弈并减少重复中间验证。
热点复盘后又完成第三轮训练治理：教师与学生共享有界训练窗口，角色通过通用配额声明
长线策略覆盖；不安全结束回合拆成 Policy 反例和 Risk-only 辅助样本；CPU 教师按数据漂移
自适应刷新，并在持久化固定锚点上执行多头回滚门禁；正式 Champion 晋级同时要求足够的
配对胜负证据、Advanced 绝对胜率、同集多头不回归和策略配额。该轮仍不赋予 Transformer
在线动作控制权。
...
当前训练实现进一步把 `roleStrategy:*` 中的 survival、finale、transform、growth、bank
阶段加入通用 frame stratum。角色策略提供者可以声明每层训练下限/上限；共享有界窗口
先执行配额选择，MLP 再按层逆频率加权，Transformer 不再无条件重复全部策略样本。
角色 ID 和配额声明值本身不参与分类。该机制用于恢复奈奈等长线角色的关键决策覆盖，
而不是把角色打法重新写成在线规则。无法满足的配额必须显式报告并阻断正式晋级。
...
训练器内部也不再把所有阶段强制串成等长屏障。最终离线验证使用有界滚动队列，Worker
完成一个战役后立即领取下一个；协调器按种子序号提交，既消除批尾空闲，又保持确定性
早停。Transformer 教师仍位于 Replay 冻结之后，但通过跨轮热启动降低串行临界路径：
CPU 中间轮允许仅评估和重标注，刷新轮做少量增量 Epoch，最终轮再执行完整收敛训练；
GPU 可以每轮增量训练。模型训练与战役模拟不会同时争抢同一 CPU 线程池。
