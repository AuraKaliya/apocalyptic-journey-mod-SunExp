...
```mermaid
flowchart TD
    A["玩家行动窗口稳定"] --> B["捕获 PlayerCombatObservation v2"]
    B --> C["建立公开历史、牌库知识与信念"]
    C --> D["展开卡牌、技能、目标与结束回合候选"]
    D --> E["无副作用合法性规则"]
    E --> F["内容语义补充"]
    F --> G["转换为纯 CombatSimulationState 与显式效果结果"]
    G --> G1["根节点决定化与风险约束 Chance-PUCT MPC"]
...
9. AI 观察不得包含真实抽牌顺序、任意动态变量、权威状态哈希或运行时对象。
10. 动作只能通过绑定当前 `ObservationId` 的 `ActionToken` 提交，旧 token 必须拒绝。
...
- 当前搜索从公开牌库信念采样未知抽牌顺序；抽牌、生成牌和交互动作后停止当前模拟分支并等待真实观察，仍不展开跨回合完整 POMDP。
