...
| 训练结果 | package v5 JSON + weights v5 binary、analysis JSON、metrics JSONL、decision-differences JSONL | 结果原子写；正式包带协议、哈希与晋级证据；拒绝结果执行轻量投影 | 只有完整通过最终门禁的包可进入发布路径；失败结果没有业务模型载荷，续训只读取独立 checkpoint |
...
| 验证 | 结果 | 能证明什么 |
|---|---|---|
| 共享 Combat AI 行为套件 | Release 零警告，631 条断言通过；共享 `net472` 运行时与三个主消费者构建通过 | 新增 Arena 高水位收缩、内存优先进程策略；候选五态、screening 不得合格、首决策差异、Teacher 新鲜度、失败输出投影及恢复来源合同仍未被绕过 |
| 稀疏推理代表负载 | 输入密度 1.5%，混合路径乘法槽位减少 98.5%；稳态约 155 µs/evaluation、1,040 B/input | 1024 逻辑维度可保留，同时让热路径按非零特征工作；该数字是测试夹具，不是全战役平均值 |
| Replay v2 微基准 | 600 Episodes 写成 3 个 shard，本机复跑约 10,228 Episodes/s；81 条 Worker 存储/损坏恢复/兼容与教师安全聚合断言通过 | 额外验证普通计划继承旧 8 GiB 峰值时预测 >9 GiB，而低内存计划不继承不匹配峰值并预测 <4 GiB；active/backlog 轮换与无丢弃合同保持通过 |
...
| Foundation Worker 发布链 | 发布产物 smoke 通过，`campaigns=248/296`、`battles=15,456`；本机实测选择 `每进程迭代=1`、`模型训练并行=12`；Worker 与 ControlCenter 已重新发布 | v16 Job/Progress/Result/Checkpoint、轮次隔离、最新续训、恢复与轻量结果写出链通过 |
