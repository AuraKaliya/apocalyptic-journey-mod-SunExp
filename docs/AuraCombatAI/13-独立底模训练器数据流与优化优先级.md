...
```mermaid
flowchart TD
    UI["ControlCenter：参数、恢复模式、档位"] --> JOB["Worker Job schema 16\n战役、规则、哈希、种子、模型"]
...
|---|---|---|---|---|
| 0. Job 构建 | 控制中心设置、训练/验证 Campaign、Ruleset、内容集合、初始 Champion | 参数归一化、生成种子分区、绑定规则与内容哈希、选择恢复语义 | `CombatFoundationWorkerJob` schema 16 | Job JSON 交给独立 Worker；路径只传工件位置，不传运行时对象；schema 15 是前代读取边界，但角色被动/搜索语义不匹配时必须 fresh；schema 13/v22/v6 仍走受限修复迁移 |
...
| 5. 特征编码 | `CombatEpisodeFrame` 状态、候选动作、Token Catalog | `partitioned-v4` 将 2048 维状态按核心/通用/玩家状态/牌区/弃牌耗尽/敌人隔离，并映射为 `int[] tokenIds + float[] values`；动作仍为 1024 维 | `CombatCompactFeatureVector` 与训练稀疏向量 | 内存与 checkpoint 都保存索引/数值列；状态碰撞目标 3%、硬上限 5%，动作硬上限 6% |
| 6. Transformer 教师 | 选中 Frames、对象 Observation、活跃语料、持久 backlog、固定 Anchor、上一代 `.pt`、已训练/已尝试 identity 水位 | 每轮合并语料并用稳定教师标注；只有冷启动、最终轮、数据漂移、新 fresh Frames 达阈值，或累计到最大刷新间隔时才重训。默认 GPU 间隔 3 轮、阈值 2048 Frames；连续更新被拒后按 2/4 倍刷新间隔退避，最终轮仍强制刷新 | 文件名保持兼容的 `world-model-v2.pt`（内部协议 v4）、独立稳定 Policy/World 教师、C# 最终报告、候选概率/固定权重蒸馏信号、trained/attempted 水位 | C# 以文件和命令行启动 Python；未重训轮使用 `annotate-only`，不推进 attempted/trained 水位；报告包含拒绝 streak、最近尝试轮与退避原因 |
...
| 9. Arena | `BestPendingArenaCandidate`（没有合格 pending 时才退回本轮诊断候选）、Champion/工作基线、Normal/Advanced 固定种子 | training-only 每轮按固定 Anchor 损失及安全 Head 选择历史最佳 pending；Arena 轮验证该历史候选，不再强制验证最新权重。Screening 通过离线 Head/碰撞/策略配额与竞技表现后才 Confirmation；默认 8 + 56 = 64 对/难度，满足非劣证据下限 | 成对胜负、深度、行为错误、Wilson/discordant evidence、候选来源 iteration、晋级结论 | 结果按稳定 pair index 提交；Confirmation 只对不可恢复 Reject 早停，Accept 必须跑满配置证据量；已消费 pending 在有效 Arena 后清空，Arena 系统故障则保留以便恢复重试 |
| 10. 轮次边界 | 最新训练模型、离线安全待验证最佳、已验证工作模型、绝对合格模型及证据、Replay 热集、训练调度、Telemetry | 四个模型槽独立写入 checkpoint；training-only 可更新 latest 和 pending，不得覆盖 working/qualified；pending 也不得被更新但更差的模型覆盖 | 可精确续训状态、下一轮位置、不会被后续未验证权重覆盖的待验证/已合格历史候选 | gzip latest + `.bak`、不可变 checkpoint 目录、`.afes` 快照；下一轮子进程恢复四种模型语义 |
| 11. 最终验收 | 历史 `AbsoluteQualifiedBest`、隔离验证种子、能力探针 | 最终轮验证历史最优合格候选，而不是最后一次训练权重；能力探针必须形成可信 `pass`，`inconclusive` 在昂贵正式验证前停止 | 正式 Champion 或拒绝；分析、指标、案例 | 合格模型和产生它的 iteration 证据必须同 ID、硬门禁为真；只有该候选能进入 package |
...
| Arena 两级淘汰 | training-only 先积累 pending；Screening 通过所有前置硬门禁才 Confirmation；开发默认每难度 8 对初筛 + 56 对确认，确认 Accept 不提前截断证据 | 最终候选来源可追溯到历史 iteration；`ArenaConfirmationPairs` 对 Accept 等于配置总量，合计达到 64 对非劣合同；不可恢复 Reject 才记录 saved pairs |
...
| Transformer 吃满 GPU | 默认 Batch 64、可配 micro-batch；≤4096 frames resident，较大集合 512-frame shards；每个 DataLoader worker 只缓存 1 个 shard；pinned memory、prefetch、AMP | 只统计实际 refresh 轮的 GPU 利用率、DataLoader wait、平均 Batch、frames/s、父进程加 worker 工作集、显存峰值、OOM backoff/CPU fallback 次数；annotate-only 轮不得误报训练吞吐 |
...
| 验证 | 结果 | 能证明什么 |
|---|---|---|
| 共享 Combat AI 行为套件 | Release 零警告，619 条断言通过 | 新增决策前权威 transition、分阶段交互合同、随机因果归属及历史 pending 候选、8+56 Arena 证据预算、教师刷新阈值、growth 正负定向配额；原有 Epoch/Arena、缓存、迁移、策略 applicability 和失败熔断合同未被绕过 |
| 稀疏推理代表负载 | 输入密度 2.3%，乘法槽位减少 97.7%；稳态约 153 µs/evaluation、1,040 B/input | 1024 逻辑维度可保留，同时让热路径按非零特征工作；该数字是测试夹具，不是全战役平均值 |
| Replay v2 微基准 | 600 Episodes 写成 3 个 shard，本机复跑约 6,800 Episodes/s；70 条 Worker 存储/损坏恢复/兼容与教师安全聚合断言通过 | 小文件数量、索引事务和 fsync 数已从逐 Episode 降到批次/分片粒度；额外验证 active 64 + backlog 16 无丢弃，并在 trained 水位推进后把 pending backlog run 轮入活跃窗口 |
...
| Foundation Worker 发布链 | 默认 Worker smoke 63 秒通过，`campaigns=6/264`、`battles=7,069`；CUDA 集成 smoke 124.7 秒通过，`campaigns=286/302`、`battles=16,855`；checkpoint 写失败为 0 | v16 Job/Progress/Result/Checkpoint、发布、恢复、CUDA 教师与结果聚合链均通过；foundation profile 的代码步骤可复现 |
| Worker/ControlCenter 与共享消费者 | 两个 Trainer Release 构建及三个主 Shared 消费者均零警告；发布 Python 与源码 SHA-256 同为 `46F73F44629ABE8550B850E78A974C21273248004ECB66160C3D2538DB3B9F83`；兼容基线 1721 项、`Aura.Shared.dll` packaging gate 通过 | 当前发布目录与游戏 Mods 内的 Worker、ControlCenter、AuraTools/Terrias Shared DLL 已逐文件 SHA-256 对齐 |
| 正式模型工件门禁 | 按设计 fail-closed：旧 `foundation-model-package-v5.json` 与 weights 已由用户移除，foundation/combat-ai profile 在 training-artifacts 步骤停止 | 不恢复、伪造或误发布旧模型；新训练必须先产出通过完整 Arena/Validation 的 v5 包，才能让整体发布档变绿 |
...
- [ ] 确认 Worker schema 16、Episode v8 / feature schema 29、在线样本 v8 / feature schema 11、交互契约与 trace v2、Transformer model/report v4、sparse dataset v3、`.afes` v5、Replay Warehouse v2、model package v5 一致；v15 只允许进入显式兼容判断，角色被动/搜索/交互语义不一致必须 fresh，v13 只允许走既有修复迁移。
