...
Changes to provider identity semantics must also bump `BuildId`, even when the
numeric protocol remains compatible, so an older first-loaded component cannot
silently keep ownership or matching behavior that the newer consumer rejects.
