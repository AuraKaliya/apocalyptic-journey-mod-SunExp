...
{
    private static IProjectionNetworkPort? transport;
    private static IProjectionNetworkPort Transport => transport ?? throw new InvalidOperationException("Projection transport is not initialized.");
    public static void ConfigureTransport(IProjectionNetworkPort value) => transport = value ?? throw new ArgumentNullException(nameof(value));
    ...
    public static bool TrySummon(ScriptExecutor self, PolymorphRoleSpec role)
    {
        ...
        var sender = TerriasRpcSender.FromAura(AuraRpcAuthorityRuntime.CreateLocalServerSender(
        ...
        string source)
    {
        ...
        var status = Transport.RequestSummon(
                transaction.RoleId,
                transaction.OwnerStatusId,
                transaction.Token,
    ...
    private static void BroadcastSummonResult(ProjectionSummonResultSnapshot result, string source)
    {
        ...
        {
            Transport.SendResult(result, source);
        }
    }
    ...
    {
        return !TerriasNetworkQueries.HasRemotePlayers() || Transport.SendState(snapshot, source);
    }
        ...
        string source)
    {
        ...
        Transport.SendTurn(new ProjectionSummonTurnSnapshot
            {
                ProtocolVersion = CompanionAuthorityService.ProjectionProtocolVersion,
                BattleEpoch = CompanionAuthorityService.BattleEpoch,
    ...
    {
        if (projection == null || !TerriasNetworkQueries.NetworkActive() || !CompanionAuthorityService.IsAuthoritative())
        {
            return;
        }
    ...
    public static void RequestRuntimeState(ProjectionOtherObj projection, string source)
    {
        ...
        Transport.RequestState(state.StatusId, state.Replication.Generation,
