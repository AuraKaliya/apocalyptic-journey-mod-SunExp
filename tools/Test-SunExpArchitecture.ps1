...
Assert-Contains $cardFrameEffectApplier "marker.BackgroundImage" "Card-frame effects must fall back to the card background image for dictionary previews without a frame node."
...
Assert-Contains $cardVisualSkinMarker "var source = FrameImage ?? BackgroundImage" "Card-frame image overlays must prefer the frame image as the alpha mask before falling back."
...
Assert-Contains $visualRegistryJson '"_FoilTex"' "Shipped visual registry must bind the Pokemon holofoil texture for foil effects."
Assert-Contains $visualRegistryJson '"_SunExpFoilMode": 2.0' "Shipped visual registry must enable the Pokemon holofoil shader mode."
Assert-Contains $visualRegistryJson 'pokemon_holo_foil.png' "Shipped visual registry must reference the local holofoil texture asset."
...
Assert-Contains $visualBundleBuilder 'material.SetFloat("_SunExpFoilMode", 2f)' "Visual bundle builder must default foil materials to Pokemon holofoil mode."
...
Assert-Contains $cardFaceEffectShaderSource "pokemonMainShine" "Foil shader must include the CodePen-style rainbow shine stack."
Assert-Contains $cardFaceEffectShaderSource "pokemonTextureShine" "Foil shader must include the CodePen-style texture shine stack."
Assert-Contains $cardFaceEffectShaderSource "pokemonGlare" "Foil shader must include the CodePen-style pointer glare layer."
Assert-Contains $cardFaceEffectShaderSource "colorDodgeBlend" "Foil shader must approximate color-dodge blending for holofoil punch."
Assert-Contains $cardFaceEffectShaderSource "hardLightBlend" "Foil shader must approximate hard-light glare blending."
