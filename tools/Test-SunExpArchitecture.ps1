...
Assert-NotContains $cardFrameEffectApplier "marker.BackgroundImage" "Card-frame effects must not fall back to card-background alpha."
...
Assert-Contains $cardFrameEffectApplier "SharedUiOverlayMaterial" "Card-frame UI effects must use transparent overlay materials for holofoil layers."
Assert-Contains $cardFrameEffectApplier "ApplyFrameImageEffectOverlay" "Card-frame UI effects must apply through an overlay layer."
Assert-Contains $cardFrameEffectApplier "ApplyFrameMeshEffectOverlay" "Card-frame mesh effects must apply through an overlay renderer instead of replacing the frame material."
...
Assert-NotContains $cardVisualSkinMarker "ApplyFrameMeshEffectMaterial" "Card-frame mesh effects must not keep a direct frame-material replacement entry point."
Assert-NotContains $cardVisualSkinMarker "originalFrameMeshMaterial" "Card-frame effects must not restore around direct frame material replacement."
Assert-Contains $cardVisualSkinMarker "EnsureFrameEffectMeshOverlay" "Card-frame mesh overlays must be created as independent overlay renderers."
Assert-Contains $cardVisualSkinMarker "DestroyFrameEffectMeshOverlay" "Card-frame mesh overlays must be cleaned up independently."
...
Assert-Contains $cardFrameEffectMaterials "OverlayMode" "Card-frame effect materials must control shader overlay mode explicitly."
Assert-Contains $cardFrameEffectMaterials "CreateOwnedOverlayMaterial" "Card-frame mesh effects must use owned overlay materials."
Assert-Contains $cardFrameEffectMaterials "ApplyFrameOnlyOverlay(material, false)" "Card-frame effect materials must keep shader-side background frame heuristics disabled."
Assert-Contains $cardFrameEffectMaterials "SunExpPerformanceSettings.CardFrameEffectQualityScale" "Card-frame effect materials must obey performance quality settings."
...
Assert-Contains $cardVisualSkinMarker 'SunExp_CardFrameEffectOverlay' "Card visual skin marker must create a named card-frame effect overlay."
Assert-Contains $cardVisualSkinMarker 'raycastTarget = false' "Card-face effect overlays must not intercept card UI input."
Assert-Contains $cardVisualSkinMarker 'DestroyFrameEffectOverlay' "Card visual skin marker must clean up card-frame effect overlays."
...
Assert-Contains $cardFaceEffectShaderSource "_SunExpFrameOnlyOverlay" "Card-face effect shader may expose frame-only overlay masking, but card-frame runtime must keep it disabled."
