...
Assert-Contains $companionSceneLifecycleRuntime "ProjectionRuntime.ClearBattle(source)" "Direct scene replacement must clear all projection runtime state."
Assert-Contains $companionSceneLifecycleRuntime "SpiritRuntime.ClearBattle(source)" "Direct scene replacement must clear all spirit runtime state."
...
Assert-Contains $companionSceneLifecycleRuntime "SchedulePostCleanupAudit(source)" "Menu-exit cleanup must schedule a next-frame residual audit."
...
Assert-Contains $companionSceneLifecycleRuntime "CaptureArtifactSnapshot(source)" "Companion cleanup must capture low-frequency boundary diagnostics before deferred Unity destruction."
...
Assert-Contains $fieldBuffHudView "private const float RootWidth = 164f" "FieldBuffHudView must use the enlarged compact vertical badge layout."
