...
$tongtianTowerNetworkSync = Read-RepoText "SunExp-Dev\Network\TongtianTowerNetworkSync.cs"
...
Assert-NotContains $tongtianTowerModeRuntime '"MapSelectUI.DataUpdate", ScheduleAbyssMapPanels' "Tongtian Tower must not request abyss panels from repeated MapSelectUI.DataUpdate ticks."
Assert-Contains $tongtianTowerNetworkSync "applyAllSlots: false" "Tongtian Tower snapshot UI refresh must be fixed-slot only."
Assert-NotContains $tongtianTowerNetworkSync "applyAllSlots: true" "Tongtian Tower snapshots must not clear editable map slots during interaction."
Assert-Contains $tongtianTowerNetworkSync "SnapshotRequestThrottleSeconds" "Tongtian Tower client snapshot requests must be throttled."
