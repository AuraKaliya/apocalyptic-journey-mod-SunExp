...
Assert-Contains $cardFrameEffectMaterials "FrameOnlyOverlay" "Card-frame effect materials must support frame-only masking for dictionary background fallbacks."
Assert-Contains $cardFrameEffectApplier "frameOnlyOverlay: marker.FrameImage == null && marker.BackgroundImage != null" "Card-frame effects must use shader frame masking when dictionary previews only expose a background image."
...
Assert-Contains $visualRegistryJson '"_SunExpFoilMode": 1.0' "Shipped visual registry must default foil effects to the laser shader mode."
...
Assert-Contains $cardFaceEffectShaderSource "_SunExpFrameOnlyOverlay" "Card-face effect shader must support frame-only overlay masking for dictionary fallbacks."
Assert-Contains $cardFaceEffectShaderSource "frameOnlyOverlayMask" "Card-face effect shader must keep fallback frame effects off the card face."
