...
$registryTestProject = Join-Path $repoRoot "Terrias-Dev.RegistryTests\Terrias-Dev.RegistryTests.csproj"
...
$requiredSources = @(
    "Terrias-Dev\GameApi\EnemyCatalogApi.cs",
    "Terrias-Dev\GameApi\EnemyCaptureSettlementApi.cs",
    "Terrias-Dev\Mechanics\SpiritCaptureService.cs",
    "Terrias-Dev\Mechanics\SpiritProfileIdentityResolver.cs",
    "Terrias-Dev\Mechanics\SpiritSummonService.cs",
    "Terrias-Dev\Hooks\SpiritRuntime.cs",
    "Terrias-Dev\Network\RpcSpiritCapture.cs",
    "Terrias-Dev\Network\RpcSpiritCompanion.cs"
)
...
$factorySource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\SpiritCardFactory.cs") -Raw -Encoding UTF8
...
Assert-True ($factorySource.Contains("TerriasIds.SpiritExchangeCountKey") -and $factorySource.Contains('Set(runtime, "TotalExCost", exchangeCount.ToString());')) "returned spirit cards must persist an independent exchange count and apply it as additive cost."
Assert-True ($factorySource.Contains("TerriasIds.SpiritIntentTurnIndexKey") -and $factorySource.Contains("TerriasIds.SpiritIntentReadyOnTurnKey")) "returned spirit cards must persist intent turn and cooldown state in Vars."
...
$cardApiSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\GameApi\CardApi.cs") -Raw -Encoding UTF8
$cardScriptsSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Scripting\CardScripts.cs") -Raw -Encoding UTF8
...
$stateStoreSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\SpiritStateStore.cs") -Raw -Encoding UTF8
...
$summonSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\SpiritSummonService.cs") -Raw -Encoding UTF8
$identityResolverSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\SpiritProfileIdentityResolver.cs") -Raw -Encoding UTF8
$spiritModelsSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\SpiritModels.cs") -Raw -Encoding UTF8
$intentRegistrySource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\SpiritIntentRegistry.cs") -Raw -Encoding UTF8
$captureRegistrySource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\SpiritCaptureRegistry.cs") -Raw -Encoding UTF8
$catalogSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\GameApi\EnemyCatalogApi.cs") -Raw -Encoding UTF8
$rpcSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Network\RpcSpiritCompanion.cs") -Raw -Encoding UTF8
$runtimeSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Hooks\SpiritRuntime.cs") -Raw -Encoding UTF8
$sceneRuntimeSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Hooks\CompanionSceneLifecycleRuntime.cs") -Raw -Encoding UTF8
$sceneApiSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\GameApi\CompanionSceneApi.cs") -Raw -Encoding UTF8
$spiritPresenterSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Hooks\Visual\SpiritAttachmentPresenter.cs") -Raw -Encoding UTF8
$spiritObjectSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\SpiritOtherObj.cs") -Raw -Encoding UTF8
$presentationComposerSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\SpiritIntentPresentationDataComposer.cs") -Raw -Encoding UTF8
$terriasIdsSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Infrastructure\TerriasIds.cs") -Raw -Encoding UTF8
$enemyCardData = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias\Data\EnemyCard\terrias.csv") -Raw -Encoding UTF8
$enemyCardText = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias\Text\EnemyCard\terrias.csv") -Raw -Encoding UTF8
$companionModelsSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\CompanionBattleModels.cs") -Raw -Encoding UTF8
$companionPlannerSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\CompanionIntentPlanner.cs") -Raw -Encoding UTF8
$companionExecutorSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\CompanionIntentExecutor.cs") -Raw -Encoding UTF8
$companionPresentationSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\CompanionIntentPresentationSnapshot.cs") -Raw -Encoding UTF8
$intentPresenterSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Hooks\Visual\ProjectionIntentPresenter.cs") -Raw -Encoding UTF8
$turnSource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\ProjectionTurnCoordinator.cs") -Raw -Encoding UTF8
$authoritySource = Get-Content -LiteralPath (Join-Path $repoRoot "Terrias-Dev\Mechanics\CompanionAuthorityService.cs") -Raw -Encoding UTF8
...
Assert-True ($spiritObjectSource.Contains("SpiritIntentPresentationDataComposer.Compose(source, presentationAdapterData)") -and $spiritObjectSource.Contains("TerriasIds.SpiritIntentAdapterCardId") -and $spiritObjectSource.Contains("TerriasIds.SpiritIntentSourceCardVar")) "spirit intent cards must preserve source presentation while replacing native enemy identity and scripts with the registered adapter."
Assert-True ($spiritObjectSource.Contains("SpiritIntentPresentationDataComposer.PresentationOverrides(presentationData)") -and -not $spiritObjectSource.Contains('IndexOf("Script"')) "spirit intent materialization must retain Description fields instead of treating the Script substring inside Description as executable code."
...
Assert-True ($presentationComposerSource.Contains('"Id"') -and $presentationComposerSource.Contains('"InitScript"') -and $presentationComposerSource.Contains('"TargetScript"') -and $presentationComposerSource.Contains('"UseScript"') -and $presentationComposerSource.Contains("new Dictionary<string, string>(source, StringComparer.Ordinal)") -and $presentationComposerSource.Contains("overrides.Remove(field)")) "spirit presentation composition must preserve source visuals while replacing and then omitting only adapter-owned executable identity fields."
Assert-True ($terriasIdsSource.Contains('SpiritIntentAdapterCardId = "Terrias_terrias_enemycard_spirit_intent_adapter"') -and $terriasIdsSource.Contains('SpiritIntentSourceCardVar = "TerriasSpiritIntentSourceCardId"')) "spirit adapter identity and source-card trace key must be centralized in TerriasIds."
...
Assert-True ($spiritPresenterSource.Contains("public static void ClearAll") -and $spiritPresenterSource.Contains('StartsWith("Terrias_SpiritVisualProxy:"')) "spirit visual cleanup must sweep registered and orphaned proxy objects."
...
Assert-True ($identityResolverSource.Contains('BaseGameRuntimePrefix = "enemy_"') -and $identityResolverSource.Contains('TerriasContentIdCompatibility.LookupCandidates(rawId, "terrias")')) "spirit identity resolution must cover base-game plus current and legacy Terrias runtime prefixes."
...
$hotspotSources = @(
    "Terrias-Dev\GameApi\EnemyCatalogApi.cs",
    "Terrias-Dev\GameApi\EnemyCaptureSettlementApi.cs",
    "Terrias-Dev\Hooks\Visual\SpiritCardFaceRuntime.cs",
    "Terrias-Dev\Mechanics\SpiritCardFactory.cs",
    "Terrias-Dev\Mechanics\SpiritSummonService.cs",
