...
PALETTE = [
    ...
    PERI, PERI_DARK, GOLD, OCHRE, BROWN, MINT, MINT2,
]

SOURCES = {
    "bankruptcy_contract": preview_dir / "goldexp-relic-ai-source-bankruptcy_contract.png",
    "old_king_coin": preview_dir / "goldexp-relic-ai-source-old_king_coin.png",
}
...
def subject_mask(im):
    ...
    for y in range(FINAL):
        for x in range(FINAL):
            ...
            if distance > 58 and max(r, g, b) > 42:
                mp[x, y] = 255
    return mask

def postprocess(name, src_path, pal):
    normalized = normalize_dark_background(Image.open(src_path))
    mask = subject_mask(normalized)

    outline_mask = mask.filter(ImageFilter.MaxFilter(7)).filter(ImageFilter.MaxFilter(7))
...
def make_sheets(finals):
    order = ["bankruptcy_contract", "old_king_coin"]

    source_sheet = Image.new("RGBA", (FINAL * 2, FINAL + 44), (20, 18, 28, 255))
    ...
    source_sheet.save(preview_dir / "goldexp-relic-icons-source-sheet.png")

    sheet = Image.new("RGBA", (FINAL * 2, FINAL + 44), (20, 18, 28, 255))
    d = ImageDraw.Draw(sheet)
    for i, name in enumerate(order):
        sheet.alpha_composite(finals[name], (i * FINAL, 0))
        d.text((i * FINAL + 8, FINAL + 12), f"{name}.png", fill=(240, 230, 200, 255))
    sheet.save(preview_dir / "goldexp-relic-icons-after.png")

    thumb = Image.new("RGBA", (160 * 2, 204), (20, 18, 28, 255))
    td = ImageDraw.Draw(thumb)
    for i, name in enumerate(order):
        t = finals[name].resize((32, 32), Image.Resampling.BOX).resize((160, 160), Image.Resampling.NEAREST)
        thumb.alpha_composite(t, (i * 160, 0))
        td.text((i * 160 + 4, 168), name[:20], fill=(240, 230, 200, 255))
    thumb.save(preview_dir / "goldexp-relic-icons-32px-check.png")
...
print("source", (preview_dir / "goldexp-relic-icons-source-sheet.png").resolve())
print("after", (preview_dir / "goldexp-relic-icons-after.png").resolve())
print("small", (preview_dir / "goldexp-relic-icons-32px-check.png").resolve())
'@
