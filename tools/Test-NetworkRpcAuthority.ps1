...
Assert-Contains $audioArbiter "MaximumPlaybackClaims = 512" "Audio presentation dedupe must remain bounded."
...
Assert-Contains $audioArbiter "pairedRemoteReplacementIds" "Remote replacement fallback must share an event-id pairing claim."
...
Assert-Contains $projectionSummon "ProjectionBuffCopyService.HydrateExact" "Projection clients must hydrate authoritative exact buff snapshots."
...
Assert-Contains $auraCgRuntime "MaxNetworkEventsPerPlayback" "Shared Skill CG playback must cap event count."
Assert-Contains $auraCgRuntime "MaxNetworkPayloadBytes" "Shared Skill CG playback must cap serialized payload bytes."
Assert-Contains $auraCgRuntime "TryBuildRegisteredNetworkRequest" "Shared Skill CG playback must resolve local registered resources from compact identities."
