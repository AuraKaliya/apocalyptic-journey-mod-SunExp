...
$sunEntry = Read-RepoText "Terrias-Dev\Entry.cs"
$sunAuthority = Read-RepoText "Terrias-Dev\Network\TerriasRpcAuthorityRuntime.cs"
$fieldNetworkSync = Read-RepoText "Terrias-Dev\Network\FieldNetworkSync.cs"
$fieldActivationIntents = Read-RepoText "Terrias-Dev\Mechanics\FieldActivationIntentCatalog.cs"
$constellationRpc = Read-RepoText "Terrias-Dev\Network\RpcConstellationStateCommit.cs"
$constellationService = Read-RepoText "Terrias-Dev\Mechanics\ConstellationService.cs"
$statusOwnershipPolicy = Read-RepoText "Terrias-Dev\Network\TerriasStatusOwnershipPolicy.cs"
$projectionRpc = Read-RepoText "Terrias-Dev\Network\RpcProjectionCompanion.cs"
$projectionSummon = Read-RepoText "Terrias-Dev\Mechanics\ProjectionSummonService.cs"
...
$endlessSeaMapPresenter = Read-RepoText "Terrias-Dev\Hooks\Ui\EndlessSeaMapViewPresenter.cs"
$sharedPayloadBudget = Read-RepoText "AuraSharedCore\AuraSharedPayloadBudget.cs"
$roleCommit = Read-RepoText "Terrias-Dev\Network\RpcSolarMemoryRoleCommit.cs"
$roleCommitApi = Read-RepoText "Terrias-Dev\GameApi\SolarMemoryRoleCommitApi.cs"
$sunSkillCgRuntime = Read-RepoText "Terrias-Dev\Features\SkillCg\TerriasSkillCgRuntime.cs"
...
Assert-Contains $modSyncRuntime 'AuraToolsRpcTransport.Send(manager, command, "ModSync.ManifestRequest")' "ModSync lobby requests must use the immediately executable RPC command path."
Assert-Contains $modSyncRuntime "TargetedManifestRequestTimeout" "ModSync targeted manifest requests must have a bounded timeout."
Assert-Contains $modSyncRuntime "BroadcastManifestRequestTimeout" "ModSync broadcast fallback requests must have a bounded timeout."
Assert-Contains $modSyncRuntime "RequestHostManifest(state, forceBroadcast: true)" "ModSync targeted timeouts must retry once through broadcast compatibility transport."
Assert-Contains $modSyncRuntime "falling back to lobby mod summary" "ModSync timeout must fall back to the lobby summary."
Assert-Contains $modSyncCommand "RequestId" "ModSync responses and chunks must carry request correlation identity."
Assert-Contains $modSyncCommand "CurrentProtocolVersion = 2" "ModSync must expose the current compatibility handshake version."
Assert-Contains $modSyncCommand "TargetedResponseDispatched" "ModSync must keep targeted response acknowledgements payload-free."
Assert-Contains $modSyncRuntime "TrySendTargetedHostModManifest" "ModSync must prefer a connection-targeted host manifest response."
Assert-Contains $modSyncRuntime "MaxTargetedManifestBytes" "ModSync targeted responses must enforce a bounded payload budget."
Assert-Contains $auraTargetedQueryTransport 'GetField("pendingQueries"' "Targeted queries must register callbacks without sending the native role-table-gated query command."
Assert-Contains $auraTargetedQueryTransport 'GetField("nextQueryId"' "Targeted queries must allocate native-compatible callback ids."
Assert-Contains $auraTargetedQueryTransport "identity?.GetComponent<PlayerManager>()" "Targeted responses must execute on the requester's PlayerManager network identity."
Assert-Contains $auraTargetedQueryTransport "RpcQueryMethod.Invoke" "Targeted responses must use the native TargetRpc serialization path."
Assert-NotContains $auraTargetedQueryTransport "AddMethodHook" "Targeted query transport must not rely on the unhookable native lobby query user-code method."
Assert-NotContains $auraTargetedQueryTransport "SendQuery(" "Targeted query transport must not enqueue the native role-table-gated query command."
...
Assert-Contains $sunEntry "TerriasRpcAuthorityRuntime.Initialize(modConfig)" "Terrias Entry must initialize RPC authority binding."
Assert-Contains $sharedAuthority "PlayerManager.UserCode_CmdReceiveRpcCommand__RpcCommandBase" "Shared RPC authority must bind the user-code receive hook."
Assert-Contains $sharedAuthority "PlayerManager.CmdReceiveRpcCommand" "Shared RPC authority must bind the generated receive hook."
Assert-Contains $sunAuthority "AuraRpcAuthorityRuntime.Register" "Terrias RPC authority must delegate hook binding to shared RPC authority."
Assert-Contains $sunAuthority "ITerriasServerBoundRpcCommand" "Terrias RPC authority must only bind server-bound commands."
Assert-Contains $sunAuthority "CreateLocalServerSender" "Terrias RPC authority must expose a local host sender for direct server paths."
...
Assert-Matches $constellationRpc "RpcConstellationRoundReward\s*:\s*RpcCommandBase,\s*ITerriasServerBoundRpcCommand" "Constellation round rewards must be host-authorized."
...
Assert-Matches $fieldNetworkSync "RpcFieldStateRequest\s*:\s*RpcCommandBase,\s*ITerriasServerBoundRpcCommand" "Field state requests must receive server-bound sender context."
Assert-Matches $fieldNetworkSync "RpcFieldStateSnapshot\s*:\s*RpcCommandBase,\s*ITerriasServerBoundRpcCommand" "Field snapshots must receive server-bound host authority."
...
Assert-Matches $projectionRpc "RpcProjectionSummonRequest\s*:\s*RpcCommandBase,\s*ITerriasServerBoundRpcCommand" "Projection summon requests must receive server-bound sender context."
...
Assert-Contains $fieldNetworkSync "TerriasStatusOwnershipPolicy.SenderOwnsStatus" "Field activation must validate the bound sender through the centralized status ownership policy."
...
Assert-Contains $fieldActivationIntents "TerriasFieldId.MoonDomain, ColumbinaHomesicknessIntent, 1" "Columbina Homesickness must authorize exactly one Moon Domain stack."
...
Assert-NotContains $fieldNetworkSync 'field != TerriasFieldId.ScorchingCanopy' "Field authorization must not hard-code Scorching Canopy as the only client-activatable field."
...
Assert-Matches $endlessSeaNetworkSync "RpcEndlessSeaStateSnapshot\s*:\s*RpcCommandBase,\s*ITerriasServerBoundRpcCommand" "Endless Sea snapshots must receive server-bound sender context."
Assert-Matches $endlessSeaNetworkSync "RpcEndlessSeaStateSnapshotRequest\s*:\s*RpcCommandBase,\s*ITerriasServerBoundRpcCommand" "Endless Sea repair requests must receive server-bound sender context."
...
Assert-Contains $endlessSeaMapPresenter "TerriasNetworkRuntime.IsClientOnly()" "Endless Sea clients must not generate missing floor plans locally."
...
Assert-Matches $endlessAbyssEvacuationRpc "RpcEndlessAbyssEvacuation\s*:\s*RpcCommandBase,\s*ITerriasServerBoundRpcCommand" "Endless Abyss evacuation must receive server-bound sender context."
...
Assert-Matches $roleCommit "RpcSolarMemoryRoleCommit\s*:\s*RpcCommandBase,\s*ITerriasServerBoundRpcCommand" "Solar Memory role commit must receive server-bound sender context."
...
Assert-Contains $roleCommitApi "TerriasRpcAuthorityRuntime.CreateLocalServerSender(source)" "Solar Memory local host commit must use the same sender authority model."
...
Assert-Contains $sunSkillCgRuntime "SkillCgArbiterRuntime.RequestCg(TerriasIds.ModId, request, syncRemote: true)" "Terrias Skill CG must delegate synchronized playback to the shared Skill CG runtime."
