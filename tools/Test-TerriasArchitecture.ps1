...
$sandroneCatRuntime = Read-RepoText "Terrias-Dev\Hooks\SandroneCatRuntime.cs"
$sandroneCatMaxHpService = Read-RepoText "Terrias-Dev\Mechanics\SandroneCatMaxHpService.cs"
$sandroneCatBattleState = Read-RepoText "Terrias-Dev\Mechanics\SandroneCatBattleState.cs"
$playerMaxHpApi = Read-RepoText "Terrias-Dev\GameApi\PlayerMaxHpApi.cs"
...
$nativeMapTextureLeaseApi = Read-RepoText "Terrias-Dev\GameApi\NativeMapTextureLeaseApi.cs"
...
$dimensionShopConfig = (Read-RepoText "Terrias\Config\DimensionShop\default.json") | ConvertFrom-Json
...
Assert-Contains $dimensionShopRuntime '!string.Equals(Convert.ToString(context.Arguments[1]), TerriasIds.DimensionShopNodeId, StringComparison.Ordinal)' "Dimension shop command routing must accept only its stable custom NodeId."
foreach ($forbidden in @("NativeMapItemNodeId", "PendingNativeNodeIds", "PrepareDimensionShopMapItem", "PrepareDimensionShopRoute", "CloseNativeBreakFallback")) {
    Assert-NotContains ($dimensionShopRuntime + $dimensionShopGameApi) $forbidden "Dimension shop must not re-enter the native Breaks route: $forbidden"
}
...
Assert-Contains $nativeMapTextureLeaseApi "TerriasResourceCache.Load<Texture2D>" "Dimension shop native presentation textures must resolve through the Terrias/shared ResourceLoader cache boundary."
Assert-NotContains $nativeMapTextureLeaseApi "Addressables." "Dimension shop must not bypass ResourceLoader with a raw Addressables key."
Assert-Contains $dimensionShopRuntime "nodes.RemoveAt(index)" "Dimension shop must fail closed by hiding only its custom candidate when no safe native presentation texture resolves."
...
Assert-Contains $dimensionShopNativeSkin "!busy && !hasTerminalOverlay" "Dimension shop insufficient-balance prices must remain in the visible native presentation state."
Assert-Contains $dimensionShopNativeSkin "if (item.CanBuy && !busy)" "Dimension shop must guard purchase semantics independently from the native price presentation state."
Assert-Contains $dimensionShopNativeSkin "ClearPrice(holder.transform)" "Dimension shop terminal offers must explicitly clear their native price presentation."
Assert-Contains $dimensionShopNativeSkin "currencyIcon.sprite = null" "Dimension shop terminal offers must release the Truth Crystal icon instead of leaving it under the status layer."
Assert-Contains $dimensionShopNativeSkin "CreateStatusOverlay(OfferVisual(holder.transform, type), item.Status)" "Dimension shop terminal shading must be constrained to the active product visual."
...
Assert-Contains $dimensionShopMapData "dimension_shop,Build,TerriasDimensionShop,-1,7" "Dimension shop map data must use its first-class custom route."
...
Assert-Contains $dimensionShopConfigSource "AuraSharedConfigStore.ReadOwner" "Dimension shop editable settings must load from AuraShared owner config."
Assert-Contains $dimensionShopConfigSource "DimensionShopBundledConfigRelativePath" "Dimension shop package defaults must remain separate from editable owner config."
...
Assert-Contains $familiarGrowthApi "AuraSharedPaths.OwnerSystemDataDirectory" "Familiar profiles must use the AuraShared owner-data tree instead of the ModsData root."
Assert-Contains $familiarGrowthApi "MigrateLoadedProfile" "Familiar profiles must copy legacy sidecars into the canonical owner-data tree."
...
Assert-Contains $runtimeHooks "SandroneCatRuntime.Initialize(modConfig)" "RuntimeHooks must initialize Sandrone Cat through an isolated hook step."
Assert-Contains $sandroneCatRuntime 'TerriasBattleLifecycleRouter.Register("SandroneCat"' "Sandrone Cat must reuse the shared battle lifecycle boundary."
Assert-NotContains $sandroneCatRuntime 'TerriasHookRegistry.Before(modConfig, "Fight' "Sandrone Cat must not register raw fight hooks outside the shared router."
Assert-Contains $sandroneCatMaxHpService "AuraBattleLifecycleRouter.EnsureBattleSession()" "Sandrone Cat start bookkeeping must use the shared combat session."
Assert-Contains $sandroneCatMaxHpService "AuraBattleLifecycleRouter.CurrentBattleSessionId" "Sandrone Cat end bookkeeping must settle the active shared combat session."
Assert-Contains $sandroneCatBattleState "startedOwners.Clear()" "Sandrone Cat state must discard prior-session start marks on restart."
Assert-Contains $sandroneCatBattleState "endedOwners.Clear()" "Sandrone Cat state must discard prior-session end marks on restart."
Assert-Contains $playerMaxHpApi "status.MaxHp = next;" "Native player Max HP mutation must remain contained inside GameApi."
Assert-Contains $playerMaxHpApi "GameSaveManager.UpdateRoles(RoleTable.Instance)" "Persistent Max HP growth must use the native role save boundary."
Assert-NotContains $sandroneCatMaxHpService "status.MaxHp =" "Sandrone Cat mechanics must delegate native state writes to GameApi."
