...
function Invoke-SourceAssertions {
    ...
    $nativeMapTextureLeaseApi = [System.IO.File]::ReadAllText((Join-Path $RepoRoot "Terrias-Dev\GameApi\NativeMapTextureLeaseApi.cs"))
    ...
    Assert-True ($runtimeHooks.IndexOf("DimensionShopRuntime.Initialize(modConfig)", [System.StringComparison]::Ordinal) -lt $runtimeHooks.IndexOf("MapNodeCardArtRuntime.Initialize(modConfig)", [System.StringComparison]::Ordinal)) "Dimension shop injection must register before generic map-node card art hooks."
    Assert-True $dimensionShopRuntime.Contains('RegisterAfter(modConfig, "Commands.load", OpenDimensionShop);') "Dimension shop must route its stable custom node after native command loading."
    Assert-True $dimensionShopRuntime.Contains('!string.Equals(Convert.ToString(context.Arguments[1]), TerriasIds.DimensionShopNodeId, StringComparison.Ordinal)') "Dimension shop command routing must accept only its stable custom NodeId."
    ...
    Assert-True ($dimensionShopRuntime.Contains('RegisterBefore(modConfig, "MapSelectUI.CreateMapItem", PreparePresentationNodes);') -and $dimensionShopRuntime.Contains("new Dictionary<string, string>(authoritative.data)") -and $dimensionShopRuntime.Contains('data["NodeId"] = nativeNodeId')) "Dimension shop must borrow a validated native building identity only on an independent UI presentation clone."
    Assert-True ($dimensionShopRuntime.Contains('RegisterAfter(modConfig, "MapItem.Init", RestorePresentationNodeIdentity);') -and $dimensionShopRuntime.Contains('RegisterBefore(modConfig, "MapSelectUI.SetNodes", NormalizeRenderedNodeIdentity);') -and $dimensionShopRuntime.Contains('node.data["NodeId"] = TerriasIds.DimensionShopNodeId')) "Dimension shop must restore every rendered clone to the stable custom identity before selection synchronization."
    Assert-True ($dimensionShopRuntime.Contains("CreatePresentationClone(") -and $nativeMapTextureLeaseApi.Contains('new("shop",') -and $nativeMapTextureLeaseApi.Contains('new("Breaks",') -and $nativeMapTextureLeaseApi.Contains('new("TerriasPresentationFallback",')) "Dimension shop must choose its presentation-only native icon from a deterministic verified fallback list."
    Assert-True ($dimensionShopRuntime.Contains('RegisterBefore(modConfig, "MapItem.Init", EnsureNativePresentationTexture);') -and $dimensionShopRuntime.Contains("NativeMapTextureLeaseApi.TryEnsurePresentationTexture") -and $nativeMapTextureLeaseApi.Contains("TerriasResourceCache.Load<Texture2D>") -and $nativeMapTextureLeaseApi.Contains("retainedTexture") -and -not $nativeMapTextureLeaseApi.Contains("Addressables.")) "Dimension shop must retain the host ResourceLoader texture path without bypassing it through direct Addressables calls."
    Assert-True ($dimensionShopRuntime.Contains("nodes.RemoveAt(index)") -and $dimensionShopRuntime.Contains("HideCandidate")) "Dimension shop must hide only its custom presentation candidate when no native Build texture is safe."
    foreach ($forbidden in @("NativeMapItemNodeId", "PendingNativeNodeIds", "PrepareDimensionShopMapItem", "PrepareDimensionShopRoute", "CloseNativeBreakFallback")) {
        Assert-True (-not ($dimensionShopRuntime + $dimensionShopGameApi).Contains($forbidden)) "Dimension shop must not re-enter the native Breaks route: $forbidden"
    }
