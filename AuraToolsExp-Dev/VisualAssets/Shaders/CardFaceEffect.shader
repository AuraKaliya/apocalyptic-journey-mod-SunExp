Shader "AuraTools/CardFrameEffectUI"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float auraUiClip(float2 position, float4 clipRect)
            {
            ...
            half3 holoRamp(float t)
            {
                ...
                half3 color = _TerriasHoloColorA.rgb * waves.r
            ...
            half3 lightenBlend(half3 baseColor, half3 layer)
            {
                return max(baseColor, layer);
            }

            half3 hardLightBlend(half3 baseColor, half3 layer)
            {
                half3 low = 2.0 * baseColor * layer;
                half3 high = 1.0 - 2.0 * (1.0 - baseColor) * (1.0 - layer);
                return saturate(lerp(low, high, step(0.5, layer)));
            }

            half3 colorDodgeBlend(half3 baseColor, half3 layer, float strength)
            {
                half3 dodged = baseColor / max(1.0 - layer * saturate(strength), 0.08);
                return saturate(lerp(baseColor, dodged, saturate(strength)));
            }

            float foilLuma(half3 color)
            {
                return dot(color, half3(0.299, 0.587, 0.114));
            }

            half3 pokemonMainShine(float2 uv, float time, float3 noise, float2 pointer, float fromCenter)
            {
                ...
                half3 rainbow = holoRamp(rainbowAxis * 4.2 + time * 0.045) * _TerriasRainbowStrength;
                ...
                half3 stripeColor = lerp(half3(0.32, 0.38, 0.78), half3(1.08, 1.12, 0.96), lightStripe);

                float radial = 1.0 - smoothstep(0.0, 0.82, distance(uv, pointer));
                half3 pointerColor = holoRamp(uv.x * 0.42 + uv.y * 0.58 + radial * 0.42 + time * 0.025);
                pointerColor *= saturate(0.35 + radial * 0.92 + fromCenter * 0.32);

                half3 blended = lerp(rainbow, rainbow * stripeColor, 0.52 + darkStripe * 0.18);
                blended = lerp(blended, pointerColor, radial * 0.42);
                return saturate(blended);
            }

            half3 pokemonTextureShine(float2 uv, float time, float3 noise, float2 pointer)
            {
                float2 foilUv = uv * float2(1.55, 0.82) + float2(time * 0.035, -time * 0.018);
                half3 foilTex = tex2D(_FoilTex, foilUv + noise.rg * 0.018).rgb;
                float foilGrain = saturate(foilLuma(foilTex) * 1.45);

                half3 pillars = holoRamp(uv.y * 7.5 + pointer.y * 2.0 + time * 0.055);
                ...
                half3 ridgeColor = lerp(half3(0.22, 0.28, 0.62), half3(0.96, 1.08, 1.04), ridge);

                half3 textureLayer = lightenBlend(pillars * (0.35 + pillarBand * 0.75), ridgeColor);
            ...
            half3 applyPokemonFoil(half3 baseColor, float2 uv, float mask, float time, float3 noise, float edge, float intensity)
            {
                ...
                half3 mainShine = pokemonMainShine(uv, time, noise, pointer, fromCenter);
                half3 textureShine = pokemonTextureShine(uv, time, noise, pointer);
                float glare = pokemonGlare(uv, pointer, fromCenter);

                half3 shineStack = colorDodgeBlend(baseColor, mainShine, 0.76 * _TerriasPrismStrength);
                shineStack = lightenBlend(shineStack, textureShine * (0.58 + fromCenter * 0.36));
                shineStack = hardLightBlend(shineStack, half3(glare, glare, glare));

                float effect = saturate((foilLuma(mainShine) * 0.56 + foilLuma(textureShine) * 0.48 + glare * 0.42 + edge * 0.22) * intensity * cardWeight);
                half3 color = lerp(baseColor, shineStack, effect);
                color += (mainShine * 0.24 + textureShine * 0.22 + half3(glare, glare, glare) * 0.14) * intensity * cardWeight;
                return saturate(color);
            }

            half3 applyFoil(half3 baseColor, float2 uv, float mask, float time, float3 noise, float edge, float intensity)
            {
                ...
                half3 holo = holoRamp(effectUv.x * 0.72 + effectUv.y * 0.58 + sheen * 0.2 + lines * 0.09 + noise.b * 0.12 + time * 0.026);
                float foil = saturate(sheen * 0.62 + lines * 0.58 + sweep * 0.74 + mirror * 0.6 + glint * 1.08 + edge * 0.42);
                half3 foilColor = lerp(baseColor, saturate(baseColor * 0.86 + holo * 1.1), _TerriasPrismStrength);
                half3 color = lerp(baseColor, foilColor, foil * intensity);
            ...
            half3 applyStardust(half3 baseColor, float2 uv, float mask, float time, float3 noise, float edge, float intensity)
            {
                ...
                half3 starColor = lerp(_TerriasStardustColorA.rgb, _TerriasStardustColorB.rgb, slowBand * 0.55 + noise.b * 0.24);
                half3 color = baseColor + starColor * dust * intensity * 0.52;
                color += starColor * sweep * intensity * 0.44;
                color = lerp(color, max(baseColor, starColor * 0.88), glint * intensity * 0.24);
                color += starColor * glint * intensity * 0.36;
                color += _TerriasStardustColorA.rgb * edge * intensity * 0.18;
                return color;
            }

            half4 buildFoilOverlay(float2 uv, float mask, float time, float3 noise, float edge, float intensity)
            {
                float2 microFlow = float2(hash21(uv * 17.0), hash21(uv * 23.0)) - 0.5;
                float2 flow = ((noise.rg * 2.0 - 1.0) + microFlow * 0.08) * _TerriasDistortion;
                float2 effectUv = uv + flow * mask;
                ...
                half3 color = holo * saturate(0.72 + sparkle * 1.05);
                color += _TerriasHoloColorA.rgb * (glint * 0.26 + edge * 0.2) * intensity;
                half4 result = half4(color, alpha * mask);
                if (_TerriasFoilMode > 1.5)
                {
                    float2 pointer = foilPointer(time);
                    float fromCenter = saturate(length(pointer - 0.5) * 2.0);
                    ...
                    half3 textureShine = pokemonTextureShine(uv, time, noise, pointer);
                    float glare = pokemonGlare(uv, pointer, fromCenter);
                    half3 shineStack = colorDodgeBlend(mainShine * 0.68, textureShine, 0.76);
                    ...
                    result = half4(saturate(shineStack * (0.86 + pokemonSparkle * 0.82) + mainShine * 0.22), pokemonAlpha * mask);
                }

                return result;
            }

            half4 buildStardustOverlay(float2 uv, float mask, float time, float3 noise, float edge, float intensity)
            {
                float dust = stardustField(uv, time, noise) * mask;
                float sweep = stardustSweep(uv, time, noise) * mask;
                float sparkle = saturate(dust * 0.48 + sweep * 0.62 + edge * 0.46);
                float slowBand = sin((uv.x * 0.72 + uv.y * 1.35 + time * 0.1) * 6.2831853) * 0.5 + 0.5;
                half3 tint = lerp(_TerriasStardustColorA.rgb, _TerriasStardustColorB.rgb, slowBand * 0.55 + noise.b * 0.24);
                half3 color = tint * saturate(0.48 + sparkle * 0.92);
                float alpha = saturate(sparkle * intensity * 0.88);
                return half4(color, alpha * mask);
            }

            half4 frag(v2f i) : SV_Target
            {
                half4 face = (tex2D(_MainTex, i.uv) + _TextureSampleAdd) * i.color;
                ...
                half3 foilColor = applyFoil(face.rgb, i.uv, mask, time, noise, edge, intensity);
                half3 starColor = applyStardust(face.rgb, i.uv, mask, time, noise, edge, intensity);
                half3 color = _TerriasEffectMode > 0.5 ? starColor : foilColor;
                half4 result = half4(color, mask);
                ...
                result.a *= auraUiClip(i.worldPosition.xy, _ClipRect);
                #endif
