...
internal static class CustomCardArtworkRuntime
{
    ...
    internal static void Activate()
    {
        ...
        hooks.BeforeRouted("DataConfig.PreCompileScripts",context=>{if(context.Target is DataConfig card)Restore(card);},"RestoreBeforeCompile");
        hooks.AfterRouted("DataConfig.RestoreData",context=>{if(context.Target is DataConfig card)Restore(card);},"RestoreAfterLoad");
        ...
        foreach(var card in Singleton<GameRuntimeData>.Instance.CardData.ToArray())
            try{Restore(card);}catch(Exception ex){AuraToolsLog.Warn("[CustomCard] saved artwork could not be restored: "+ex.Message);}
    }
    internal static void Restore(DataConfig card)
    {
        ...
        if(!card.data.TryGetValue("AuraToolsCustomCardVersion",out var version)||version!="1")return;
