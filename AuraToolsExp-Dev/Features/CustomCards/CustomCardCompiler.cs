...
public static class CustomCardCompiler
{
    ...
    public static CustomCardCompilation Compile(CustomCardDocument document, Func<string, string?>? buffName = null)
    {
        ...
        if (document.Rules == null || document.Rules.Count == 0 || document.Rules.Count > 12) { Error(c, "", "作品需要 1～12 组触发规则。"); return c; }
        ...
        foreach (var rule in document.Rules)
        {
            ...
            if (!Enum.IsDefined(typeof(CardRuleTrigger), rule.Trigger)) Error(c, rule.Id, "未知触发时机。");
            if (rule.MaximumTriggers < 0 || rule.MaximumTriggers > 999) Error(c, rule.Id, "触发次数需为 0～999；0 表示本场不限次数。");
            if (rule.MaximumTriggersPerRound < 0 || rule.MaximumTriggersPerRound > 999) Error(c, rule.Id, "每回合触发次数需为 0～999；0 表示不限次数。");
        ...
        foreach (var trigger in new[] { CardRuleTrigger.Draw, CardRuleTrigger.Use, CardRuleTrigger.Discard })
        {
            ...
            foreach (var rule in document.Rules.Where(r => r.Trigger == trigger || (trigger == CardRuleTrigger.Use && r.Trigger >= CardRuleTrigger.AfterUseRoundStart)))
                emitter.Rule(rule);
        ...
        c.Description = CustomCardDescription.Describe(document);
        ...
        foreach (var b in blocks)
        {
            ...
            switch (b.Kind)
            {
                case CardBlockKind.Effect:
                    ...
                    if (CustomCardNames.PlayerEffect(b.Effect) && b.Object?.Kind != CardObjectKind.Self) Error(c, b.Id, "能量、牌堆和回合效果只能作用于自己。");
    ...
    {
        if (++nodes > MaximumNodes || depth > MaximumDepth) { Error(c, id, "表达式过于复杂。"); return; }
        ...
        if (v.Kind == CardValueKind.Variable && (!scope.Variables.TryGetValue(v.VariableId ?? "", out var isObject) || isObject)) Error(c, id, "引用的数值尚未定义，或不在这个分支内。");
        if (v.Kind == CardValueKind.Read || v.Kind == CardValueKind.Exists)
        {
            ...
            if (v.Kind == CardValueKind.Read)
            {
                ...
                if (v.Field >= CardDataField.Energy && v.Object?.Kind != CardObjectKind.Self) Error(c, id, "玩家能量、牌堆和本牌费用仅从自己读取。");
        ...
        foreach (var input in v.Inputs) Value(input, v.Kind == CardValueKind.And || v.Kind == CardValueKind.Or || v.Kind == CardValueKind.Not, scope, target, depth + 1, ref nodes, id, c, buffName);
    ...
    private static void Object(CardObjectReference? o, Scope scope, bool target, bool single, string id, CustomCardCompilation c)
    {
        ...
        if (single && !CustomCardNames.Single(o.Kind)) Error(c, id, "此处需要单个对象，请先逐个遍历或记住对象。");
        if (o.Kind == CardObjectKind.Current && !scope.Current) Error(c, id, "当前遍历对象只能在逐个执行内部使用。");
        if (o.Kind == CardObjectKind.Target && !target) Error(c, id, "只有需要选中目标的卡牌在使用时可引用选中目标。");
        if (o.Kind == CardObjectKind.Saved && (!scope.Variables.TryGetValue(o.VariableId ?? "", out var isObject) || !isObject)) Error(c, id, "引用的对象尚未定义，或不在这个分支内。");
    }

    private static void Buff(string id, string node, CustomCardCompilation c, Func<string, string?>? name)
    {
        if (string.IsNullOrWhiteSpace(id) || id.Length > 200 || id.Any(char.IsControl)) { Error(c, node, "请选择状态。"); return; }
        var display = name?.Invoke(id);
        if (name != null && display == null) Error(c, node, "状态不存在或未加载：" + id);
    ...
    private static void Error(CustomCardCompilation c, string id, string message) { if (c.Issues.Count < 40) c.Issues.Add(new() { NodeId = id, Message = message }); }
    ...
    private sealed class Emitter
    {
        ...
        internal void Rule(CustomCardRule r)
        {
            ...
            if (r.Trigger < CardRuleTrigger.AfterUseRoundStart) b.AppendLine("do local c=__context(); if " + Expr(r.Condition) + " then " + root + "(c,function() end) end end");
            else
            {
                var evt = r.Trigger == CardRuleTrigger.AfterUseRoundStart ? "StartRound" : r.Trigger == CardRuleTrigger.AfterUseRoundEnd ? "EndRound" : "Hurt";
                b.AppendLine("__listen(" + Quote(evt) + ", " + r.MaximumTriggers + ", " + r.MaximumTriggersPerRound + ", function(c) return " + Expr(r.Condition) + " end, " + root + ")");
            }
        }
        ...
        {
            var then = n.Kind == CardBlockKind.If || n.Kind == CardBlockKind.Repeat || n.Kind == CardBlockKind.ForEach ? Sequence(n.Then) : "";
            var otherwise = n.Kind == CardBlockKind.If ? Sequence(n.Else) : "";
            var name = Next();
        ...
        private static string Subject(CardObjectReference o) => o.Kind switch { CardObjectKind.Self => "__self.Self", CardObjectKind.Target => "c.target", CardObjectKind.Current => "c.current", CardObjectKind.Saved => "c.vars[" + Quote(o.VariableId) + "]", CardObjectKind.RandomEnemy or CardObjectKind.RandomFriend => "__objects(" + Quote(o.Kind.ToString()) + ")[1]", _ => "nil" };
        ...
        private static string Expr(CardValue v)
        {
