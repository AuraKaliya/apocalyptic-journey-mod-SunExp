...
internal static class ReplayCanonicalJsonV10
{
    ...
    internal static string EventChainHash(string previousHash, ReplayTimelineEventV10 value)
    {
        var token = JObject.FromObject(value, Serializer);
        token[nameof(ReplayTimelineEventV10.EventChainHashAfter)] = "";
        var payloadHash = Sha256(Normalize(token).ToString(Formatting.None));
        return Sha256Text((previousHash ?? "") + "\n" + payloadHash);
    }

    internal static string DocumentHash(ReplayDocumentV10 document)
    {
        var token = JObject.FromObject(document, Serializer);
        if (token[nameof(ReplayDocumentV10.Header)] is JObject header)
        {
            header[nameof(ReplayDocumentHeaderV10.DocumentSha256)] = "";
        }
