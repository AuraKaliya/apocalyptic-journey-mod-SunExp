...
internal static class ReplayAssetContractV17
{
    internal const long MaximumSingleAssetBytes = 256L * 1024L * 1024L;
    internal const int MaximumImageDimension = 8192;

    internal static string Validate(ReplayAssetV17? asset, bool requirePayload)
    {
        ...
        if ((asset.Usage?.Length ?? 0) > ReplayLimitsV17.MaximumTextLength) return "usage-invalid";
        ...
        if (payload.LongLength != asset.ByteLength
            || !string.Equals(ReplayCanonicalJsonV17.Sha256(payload), asset.Sha256, StringComparison.OrdinalIgnoreCase))
    ...
    private static string ValidatePng(ReplayAssetV17 asset, byte[] payload)
    {
    ...
    private static string ValidateWave(ReplayAssetV17 asset, byte[] payload)
    {
        if (!ReplayPcm16WaveContractV17.TryRead(payload, out var wave, out _)) return "wave-invalid";
...
internal sealed class ReplayAssetPayloadSetV17
{
    public List<ReplayAssetPayloadV17> Items { get; set; } = new();
}

internal sealed class ReplayAssetPayloadV17
{
    public string Sha256 { get; set; } = "";
    public byte[] Payload { get; set; } = Array.Empty<byte>();
}

internal static class ReplayAssetPayloadTransferV17
{
    internal static ReplayAssetPayloadSetV17 Capture(ReplayDocumentV17 document)
    {
        return new ReplayAssetPayloadSetV17
        {
            Items = (document?.Assets ?? new List<ReplayAssetV17>())
                .OrderBy(item => item.Sha256, StringComparer.Ordinal)
                .Select(item => new ReplayAssetPayloadV17
                {
                    Sha256 = item.Sha256,
    ...
    internal static void AttachAndValidate(ReplayDocumentV17 document, ReplayAssetPayloadSetV17 payloadSet)
    {
