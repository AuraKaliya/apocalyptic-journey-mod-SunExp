...
internal sealed class ReplayTransactionLedgerEntryV12
{
...
internal sealed class ReplayTransactionLedgerV12
{
    private readonly Dictionary<string, ReplayTransactionLedgerEntryV12> entries = new(StringComparer.Ordinal);
    private long openSequence;

    internal int OpenCount => entries.Values.Count(item => !item.Terminal);

    internal IReadOnlyList<ReplayTransactionLedgerEntryV12> OpenEntries => entries.Values
        ...
        string parentTransactionId = "")
    {
        ...
        entries.Add(transactionId, new ReplayTransactionLedgerEntryV12
        {
    ...
    internal bool TryBindPresentation(
        string actorId,
        string sourceInstanceId,
        ...
        out string rejection)
    {
        var candidates = entries.Values.Where(item => !item.Terminal)
            ...
            .Where(item => string.IsNullOrWhiteSpace(sourceInstanceId)
                           || string.IsNullOrWhiteSpace(item.SourceInstanceId)
    ...
    private ReplayTransactionLedgerEntryV12 RequireOpen(string transactionId)
    {
    ...
    private int Depth(ReplayTransactionLedgerEntryV12 value)
    {
        var depth = 0;
