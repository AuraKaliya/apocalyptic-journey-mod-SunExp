...
internal readonly struct ReplayPcm16WaveInfoV12
{
    internal ReplayPcm16WaveInfoV12(int sampleRate, int channels, int dataOffset, int dataLength)
    {
...
internal static class ReplayPcm16WaveContractV12
{
    ...
    internal static bool TryRead(byte[]? payload, out ReplayPcm16WaveInfoV12 info, out string error)
    {
        info = default;
        error = "";
        ...
        info = new ReplayPcm16WaveInfoV12(sampleRate, channels, HeaderBytes, dataLength);
        return true;
    }

    internal static float[] DecodeSamples(byte[] payload, ReplayPcm16WaveInfoV12 info, int maximumSampleValues)
    {
