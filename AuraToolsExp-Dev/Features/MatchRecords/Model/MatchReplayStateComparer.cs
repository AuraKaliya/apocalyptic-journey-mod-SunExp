...
internal static class MatchReplayStateComparer
{
        ...
        IEnumerable<MatchReplayStatusState> actual)
    {
        ...
        foreach (var id in expectedMap.Keys.Union(actualMap.Keys, StringComparer.Ordinal).OrderBy(value => value))
        {
            ...
            Add(diff, "status[" + id + "].owner", left.ContentOwnerModId, right.ContentOwnerModId);
            Add(diff, "status[" + id + "].content", left.ContentId, right.ContentId);
            Add(diff, "status[" + id + "].kind", left.EntityKind, right.EntityKind);
            Add(diff, "status[" + id + "].slot", left.SlotIndex, right.SlotIndex);
