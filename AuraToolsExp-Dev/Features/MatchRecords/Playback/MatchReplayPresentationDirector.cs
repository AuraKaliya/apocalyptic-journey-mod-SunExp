...
internal static class MatchReplayPresentationDirector
{
    ...
    private static void PlayStatusAnimation(MatchReplayActionFrame frame)
    {
        ...
        if (manager == null || string.IsNullOrWhiteSpace(frame.ActorId)
                            ...
                            || actor == null)
        {
            throw new InvalidOperationException("Recorded native action actor is unavailable: " + frame.ActorId);
        }
        ...
        foreach (var targetState in targetStates)
        {
            ...
            else if (!string.IsNullOrWhiteSpace(targetState.TargetId))
            {
                throw new InvalidOperationException("Recorded native action target is unavailable: " + targetState.TargetId);
            }
        }
        ...
        if (fightUi == null)
        {
            throw new InvalidOperationException("Native FightUI is unavailable for action presentation.");
        }
        ...
        if (!MatchReplayNativePresentationApi.TryPlay(
                ...
                out var detail))
        {
            throw new InvalidOperationException("Native action presentation failed: " + detail);
        }
    }
