...
- 卡牌数量：30
- 遗物数量：13
- 事件数量：1 个地图日耀事件入口，6 段乌娜记录与 1 个重复节点
...
- 地图事件 `solar_event`：显示为“日耀事件”，属性文本为“白曜余烬”。进入后按进度推进 `Sub_wuna_event_01` 至 `Sub_wuna_event_06`，讲述乌娜、第二日轮、白曜圣庭与曜日魔女的阶段式记录；当前路线完成后进入 `Sub_wuna_event_repeat`。
