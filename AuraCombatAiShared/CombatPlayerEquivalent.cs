...
public static class CombatDecisionExecutionBindingProtocol
{
    ...
    private static CombatDecision CloneDecision(CombatDecision? source)
    {
        ...
        return new CombatDecision
        {
            ...
            SearchEvidence = source.SearchEvidence,
            PolicyAmbiguity = source.PolicyAmbiguity,
            SemanticCoverageRisk = source.SemanticCoverageRisk,
            OutcomeUncertainty = source.OutcomeUncertainty,
...
public static class CombatPublicFeaturePolicy
{
    ...
    private static readonly HashSet<string> UnitKeys =
        new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            CombatDynamicDamageProjection.Strength,
            "DefendPercent",
            "MaxChangeHp",
            "HealMultiplier",
            CombatEnemyPriorityPolicy.SummonPotentialFeature,
            CombatEnemyPriorityPolicy.SupportPotentialFeature,
            CombatEnemyPriorityPolicy.StrategicPriorityFeature,
            CombatEnemyPriorityPolicy.ExpectedThreatFeature
        };
...
public static class CombatPlayerObservationBoundary
{
        ...
        CombatActionSemantics? source)
    {
        ...
        return new CombatActionSemantics
        {
            ...
            DirectDamage = Finite(source.DirectDamage),
            ContextDamage = Finite(source.ContextDamage),
            DirectSelfHpLoss = Finite(source.DirectSelfHpLoss),
            ContextSelfHpLoss = Finite(source.ContextSelfHpLoss),
            DirectHeal = Finite(source.DirectHeal),
            ContextHeal = Finite(source.ContextHeal),
            ObservedNetHpDelta = Finite(source.ObservedNetHpDelta),
            MinimumHpDuringAction = Finite(source.MinimumHpDuringAction),
            ...
            Interaction = source.Interaction?.Normalize(),
